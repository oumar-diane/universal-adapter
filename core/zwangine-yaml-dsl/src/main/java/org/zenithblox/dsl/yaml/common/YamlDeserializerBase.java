...
import org.zenithblox.LineNumberAware;
...
public abstract class YamlDeserializerBase<T> extends YamlDeserializerSupport implements ConstructNode {
    ...
    protected void onNewTarget(Node node, T target, int line) {
        ...
        if (ctx != null && target instanceof ZwangineContextAware) {
            ZwangineContextAware.trySetZwangineContext(target, ctx.getZwangineContext());
        }
