...
public abstract class YamlDeserializerEndpointAwareBase<T> extends YamlDeserializerBase<T> {
    ...
    protected void setProperties(T target, MappingNode node) {
        ...
        for (NodeTuple tuple : node.getValue()) {
            ...
            final String propertyName = StringHelper.zwangineCaseToDash(key.getValue()).toLowerCase(Locale.US);
        ...
        if (parameters != null) {
            setEndpointUri(dc.getZwangineContext(), node, target, parameters);
        }
    }

    protected abstract void setEndpointUri(ZwangineContext context, Node node, T target, Map<String, Object> parameters);
}
