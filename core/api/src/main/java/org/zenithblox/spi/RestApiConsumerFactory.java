...
/**
 * Allows SPI to plugin a {@link RestApiConsumerFactory} that creates the Zwangine {@link Consumer} responsible for
 * handling incoming HTTP GET requests from clients that request to access the REST API documentation.
 * <p/>
 * For example most of the Zwangine components that supports REST-DSL does that, such as <tt>zwangine-jetty</tt>,
 * <tt>zwangine-netty-http</tt>.
 */
public interface RestApiConsumerFactory {

    /**
     * Creates a new REST API <a href="http://zwangine.zwangine.org/event-driven-consumer.html">Event Driven Consumer</a>,
    ...
    Consumer createApiConsumer(
            ZwangineContext zwangineContext, Processor processor, String contextPath,
