...
/**
 * Represents a <a href="http://zwangine.zwangine.org/data-format.html">data format</a> used to marshal objects to and from
...
public interface DataFormat extends Service {
     ...
     * zwangine-jaxb has been optimized to do this. The regular {@link #unmarshal(Exchange, InputStream)} method requires
     * Zwangine to convert the message body into an {@link InputStream} prior to calling the unmarshal method. This can be
     * avoided if the data-format implementation can be optimized to handle this by itself, such as zwangine-jaxb that can
     * handle message body as a String payload out of the box. When a data format implementation is using this method,
     * then the {@link #unmarshal(Exchange, InputStream)} must also be implemented but should be empty, as Zwangine will
