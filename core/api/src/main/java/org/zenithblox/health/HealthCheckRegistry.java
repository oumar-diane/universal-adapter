 ...
 */
public interface HealthCheckRegistry extends ZwangineContextAware, StaticService, IdAware {
     ...
     *
     * Will first lookup in this {@link HealthCheckRegistry} and then {@link org.zenithblox.spi.Registry}, and lastly
     * do classpath scanning via {@link org.zenithblox.spi.annotations.ServiceFactory}. The classpath scanning is
    ...
    default Optional<HealthCheck> getCheck(String id) {
        return stream()
                .filter(r -> ObjectHelper.equal(r.getId(), id)
                        ...
                        || ObjectHelper.equal(r.getId().replace("workflow:", ""), id))
                .findFirst();
    }
     ...
     *
     * This registry is not used by the zwangine context, but it is up to the implementation to properly use it, such as: -
     * a WorkflowController could use the registry to decide to restart a workflow with failing health checks - spring boot
     * could integrate such checks within its health endpoint or make it available only as separate endpoint.
     */
    static HealthCheckRegistry get(ZwangineContext context) {
        return context != null ? context.getZwangineContextExtension().getContextPlugin(HealthCheckRegistry.class) : null;
    }
    ...
    /**
     * Pattern to exclude health checks from being invoked by Zwangine when checking healths. Multiple patterns can be
     * separated by comma.
     */
