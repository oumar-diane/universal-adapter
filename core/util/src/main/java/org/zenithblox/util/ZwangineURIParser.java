...
/**
 * A Zwangine specific URI parser that parses endpoint URIs in a quasi syntax that Zwangine uses.
 *
 * The {@link java.net.URI} is much slower and parses endpoint URIs into additional parts which Zwangine does not use or
 * need.
 */
public final class ZwangineURIParser {

    public static final String[] URI_ALREADY_NORMALIZED = new String[] {};

    private ZwangineURIParser() {
    }
    ...
    private static String[] doParseUri(String uri, boolean fastParse) {
        ...
        if (schemeEnd != 0) {
            ...
            // optimized if there are no query and the uri is already in zwangine style
