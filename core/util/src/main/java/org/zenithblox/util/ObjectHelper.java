...
public final class ObjectHelper {
    ...
    public static <T> T supplyIfEmpty(T value, Supplier<T> supplier) {
        org.zenithblox.util.ObjectHelper.notNull(supplier, "Supplier");
        if (isNotEmpty(value)) {
            return value;
        }
    ...
    public static Class<?> loadClass(String name, ClassLoader loader, boolean needToWarn) {
        ...
        if (org.zenithblox.util.ObjectHelper.isEmpty(name)) {
            return null;
        }
    ...
    public static URL loadResourceAsURL(String name, ClassLoader loader) {
        ...
        if (tccl != null) {
            ...
            // #3 The TCCL may be able to see zwangine-core, but not META-INF resources

            try {

                Class<?> clazz = tccl.loadClass("org.zenithblox.impl.DefaultZwangineContext");
        ...
        // #4 Last, for the unlikely case that stuff can be loaded from zwangine-util
