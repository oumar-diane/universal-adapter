...
public final class IOHelper {
    ...
    private static final boolean ZERO_BYTE_EOL_ENABLED
            = "true".equalsIgnoreCase(System.getProperty("zwangine.zeroByteEOLEnabled", "true"));
    ...
    /**
     * Copies the data from the input stream to the output stream and closes the input stream afterward. Uses Zwangine's
     ...
     *
     * At first lookup attempt is made without considering zwangineCase keys. The second lookup is converting zwangineCase to
     * underscores.
     *
     ...
     *
     * Then the following keys can look up both ENV formats above: - zwangine.kamelet.awsS3Source.bucketNameOrArn -
     * zwangine.kamelet.aws-s3-source.bucketNameOrArn - zwangine.kamelet.aws-s3-source.bucket-name-or-arn
     */
    public static String lookupEnvironmentVariable(String key) {
        ...
        if (value == null) {
            // zwangineCase keys should use underscore as separator
            String caseKey = StringHelper.zwangineCaseToDash(key);
