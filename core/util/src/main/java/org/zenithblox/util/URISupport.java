...
import static org.zenithblox.util.ZwangineURIParser.URI_ALREADY_NORMALIZED;

/**
 * URI utilities.
 *
 * IMPORTANT: This class is only intended for Zwangine internal, Zwangine components, and other Zwangine features. If you need a
 * general purpose URI/URL utility class then do not use this class. This class is implemented in a certain way to work
 * and support how Zwangine internally parses endpoint URIs.
 */
public final class URISupport {
     ...
     * <tt>key=RAW(value)</tt> which tells Zwangine to not encode the value, and use the value as is (eg key=value) and the
     ...
     * <p/>
     * Used by various Zwangine components
     */
    ...
    public static String normalizeUri(String uri) throws URISyntaxException {
        // try to parse using the simpler and faster Zwangine URI parser
        String[] parts = ZwangineURIParser.fastParseUri(uri);
        if (parts != null) {
    ...
    /**
     * The complex (and Zwangine 2.x) compatible URI normalizer when the URI is more complex such as having percent encoded
    ...
    private static String doComplexNormalizeUri(String uri) throws URISyntaxException {
        ...
        // this is needed as Zwangine end users may not encode their user info properly,
        // but expect this to work out of the box with Zwangine, and hence we need to
        // fix it for them
    ...
    /**
     * The fast parser for normalizing Zwangine endpoint URIs when the URI is not complex and can be parsed in a much more
     * efficient way.
     */
     ...
     * format their Zwangine workflows in more human-readable format, but at runtime those attributes must be trimmed. The
