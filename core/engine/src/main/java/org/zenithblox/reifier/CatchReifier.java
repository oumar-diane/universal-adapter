...
public class CatchReifier extends ProcessorReifier<CatchDefinition> {
    ...
    public CatchReifier(Workflow workflow, ProcessorDefinition<?> definition) {
        super(workflow, CatchDefinition.class.cast(definition));
    }
    ...
    public Processor createProcessor() throws Exception {
        ...
        CatchProcessor processor
                = new CatchProcessor(getZwangineContext(), definition.getExceptionClasses(), childProcessor, when);
        // inject id
        String id = getId(definition);
        processor.setId(id);
        processor.setWorkflowId(workflow.getWorkflowId());
    ...
    protected List<Class<? extends Throwable>> createExceptionClasses() throws ClassNotFoundException {
        // must use the class resolver from ZwangineContext to load classes to
        // ensure it can
        ...
        for (String name : list) {
            Class<Throwable> type = zwangineContext.getClassResolver().resolveMandatoryClass(name, Throwable.class);
            answer.add(type);
        }
        return answer;
    }
