...
public class DefaultAsyncProcessorAwaitManager extends ServiceSupport implements AsyncProcessorAwaitManager, StaticService {
    ...
    public void await(Exchange exchange, CountDownLatch latch) {
        ReactiveExecutor reactiveExecutor = exchange.getContext().getZwangineContextExtension().getReactiveExecutor();
    ...
    public void interrupt(Exchange exchange) {
        ...
        if (entry != null) {
            try {
                ...
                // dump a workflow stack trace of the exchange
                String workflowStackTrace = MessageHelper.dumpMessageHistoryStacktrace(exchange, exchangeFormatter, false);
                sb.append(workflowStackTrace);
    ...
    private static String dumpBlockedThread(AwaitThread entry) {
        ...
        sb.append(style("WorkflowId:")).append(safeNull(entry.getWorkflowId())).append("\n");
