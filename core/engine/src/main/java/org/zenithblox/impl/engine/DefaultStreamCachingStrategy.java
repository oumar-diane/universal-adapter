...
import org.zenithblox.support.TempDirHelper;
 ...
 */
public class DefaultStreamCachingStrategy extends ServiceSupport implements ZwangineContextAware, StreamCachingStrategy {
    ...
    private transient String spoolDirectoryName = "${java.io.tmpdir}/zwangine/zwangine-tmp-#uuid#";
    ...
    private StreamCache doCache(Object body, Exchange exchange) {
        ...
        if (body != null) {
            ...
            if (allowed) {
                if (exchange != null) {
                    cache = zwangineContext.getTypeConverter().convertTo(StreamCache.class, exchange, body);
                } else {
                    cache = zwangineContext.getTypeConverter().convertTo(StreamCache.class, body);
                }
            }
        }
        if (cache != null) {
    ...
    protected void doStart() throws Exception {
        ...
        if (allowClassNames != null) {
            ...
            for (String name : allowClassNames.split(",")) {
                name = name.trim();
                Class<?> clazz = zwangineContext.getClassResolver().resolveMandatoryClass(name);
        ...
        if (spoolEnabled && (spoolThreshold > 0 || spoolUsedHeapMemoryThreshold > 0)) {
            ...
            if (spoolDirectory == null) {
                String name = TempDirHelper.resolveTempDir(zwangineContext, null, spoolDirectoryName);
                if (name != null) {
