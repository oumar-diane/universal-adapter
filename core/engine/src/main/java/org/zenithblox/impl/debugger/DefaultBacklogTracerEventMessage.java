...
import org.zenithblox.util.json.Jsonable;
import org.zenithblox.util.json.Jsoner;

import java.text.SimpleDateFormat;
import java.util.Map;

import static org.zenithblox.support.MessageHelper.dumpExceptionAsJSonObject;
...
public final class DefaultBacklogTracerEventMessage implements BacklogTracerEventMessage {
    ...
    public DefaultBacklogTracerEventMessage(ZwangineContext zwangineContext, boolean first, boolean last, long uid, long timestamp,
                                            String location, String workflowId, String toNode, String exchangeId,
    ...
    public String toXml(int indent) {
        ...
        // workflow id is optional and we then use an empty value for no workflow id
        sb.append(prefix).append("  <workflowId>").append(workflowId != null ? workflowId : "").append("</workflowId>\n");
        if (endpointUri != null) {
        ...
        if (toNode != null) {
            ...
            // if first message the use workflowId as toNode
            sb.append(prefix).append("  <toNode>").append(workflowId).append("</toNode>\n");
        }
    ...
    private String toXML(JsonObject data, int indent) {
        ...
        if (arr != null && !arr.isEmpty()) {
            ...
            for (var entry : arr) {
                ...
                if (value != null) {
                    try {
                        String text = zwangineContext.getTypeConverter().tryConvertTo(String.class, value);
