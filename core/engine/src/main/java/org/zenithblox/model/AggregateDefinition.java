...
import org.zenithblox.builder.PredicateClause;
...
public class AggregateDefinition extends OutputDefinition<AggregateDefinition>
    ...
    @Metadata(label = "advanced", javaType = "org.zenithblox.processor.aggregate.AggregateController")
    private String aggregateController;
    @Metadata(javaType = "org.zenithblox.spi.AggregationRepository")
     ...
     * org.zenithblox.processor.aggregate.MemoryAggregationRepository
     */
     ...
     * Long as result. If both are set Zwangine will fallback to use the fixed value if the Expression result was null or
     ...
     *
     * @param completionSizeExpression the completion size as an {@link org.zenithblox.Expression} which is evaluated
    ...
    /**
     * To use a {@link org.zenithblox.processor.aggregate.AggregateController} to allow external sources to control
     * this aggregator.
     */
    ...
    /**
     * Enables the batch completion mode where we aggregate from a {@link org.zenithblox.BatchConsumer} and aggregate
     * the total number of exchanges the {@link org.zenithblox.BatchConsumer} has reported as total by checking the
     * exchange property {@link org.zenithblox.Exchange#BATCH_COMPLETE} when its complete.
     * <p/>
     ...
     * set Zwangine will fallback to use the fixed value if the Expression result was null or 0.
     *
     * @param  completionSize the completion size as an {@link org.zenithblox.Expression} which is evaluated as a
     *                        {@link Integer} type
     * @return                builder
     */
    ...
    /**
     * A repeating period in millis by which the aggregator will complete all current aggregated exchanges. Zwangine has a
     ...
     * ({@link org.zenithblox.TimeoutMap}).
     * <p/>
     ...
     * <p/>
     * Will by default use {@link org.zenithblox.processor.aggregate.MemoryAggregationRepository}
     *
     ...
     * exchanges before its stopped, when stopping ZwangineContext or the workflow using it.
     */
     ...
     * Zwangine should use a thread pool with multiple threads for concurrency. If no custom thread pool has been specified
     * then Zwangine creates a default pool with 10 concurrent threads.
     */
     ...
     * implementing {@link org.zenithblox.spi.OptimisticLockingAggregationRepository}.
     */
