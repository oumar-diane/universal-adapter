...
public final class ProcessorDefinitionHelper {
    ...
    /**
     * Gets the workflow definition the given node belongs to.
     *
     * @param  node the node
     * @return      the workflow, or <tt>null</tt> if not possible to find
     */
    public static WorkflowDefinition getWorkflow(NamedNode node) {
        if (node == null) {
            return null;
        }
        ...
        if (def instanceof WorkflowDefinition rd) {
            return rd;
        } else {
            // not found
            return null;
    ...
    public static String getWorkflowId(NamedNode node) {
        WorkflowDefinition workflow = getWorkflow(node);
     ...
     *                         {@link org.zenithblox.model.OptionalIdentifiedDefinition#hasCustomIdAssigned()}
     * @param  includeAbstract whether to include abstract nodes (ie.
     *                         {@link org.zenithblox.model.ProcessorDefinition#isAbstract()}
     ...
     * StackTrace so this should only be used when Zwangine Debugger is enabled.
     *
     * @param node the node
     */
    public static void prepareSourceLocation(Resource resource, NamedNode node) {
        ...
        if (number < 0) {
            ...
            for (int i = 1; i < st.length; i++) {
                ...
                if (!e.getClassName().startsWith("org.zenithblox.model") &&
                        !e.getClassName().startsWith("org.zenithblox.builder.WorkflowBuilder") &&
                        !e.getClassName().startsWith("org.zenithblox.dsl")) {
                    // when we are no longer in model/WorkflowBuilder, we have found the location:line-number
    ...
    /**
     * Returns the level of the node in the workflow tree. Level 1 is the root level, level 2 is a child of an EIP, and so
     * forth
     */
    ...
    public static boolean shouldWrapInErrorHandler(
            ZwangineContext context, ProcessorDefinition<?> definition,
        ...
        if (definition instanceof TryDefinition || definition instanceof CatchDefinition
            ...
            Boolean isShareUnitOfWork = ZwangineContextHelper.parseBoolean(context, def.getShareUnitOfWork());
