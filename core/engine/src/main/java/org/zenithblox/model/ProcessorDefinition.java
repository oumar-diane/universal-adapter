...
public abstract class ProcessorDefinition<Type extends ProcessorDefinition<Type>> extends OptionalIdentifiedDefinition<Type>
    ...
    private WorkflowConfigurationDefinition workflowConfiguration;
    private final List<InterceptStrategy> interceptStrategies = new ArrayList<>();
    private final int index;
    private Boolean inheritErrorHandler; // used for zwangine-jta
    ...
    protected ProcessorDefinition(ProcessorDefinition source) {
        ...
        this.workflowConfiguration = source.workflowConfiguration;
     ...
     * <p/>
     * This can be used to know the order in which the definition was created when assembled as a workflow.
     *
    ...
    /**
     * Whether this definition can only be added as top-level directly on the workflow itself (such as
     ...
     * <p/>
     * Regular definitions is what is part of the workflow, such as ToDefinition, WireTapDefinition and the likes.
     * <p/>
     ...
     * <p/>
     * This information is used in zwangine-spring to let Zwangine work a bit on the model provided by JAXB from the Spring
     ...
     * <p/>
     * When a definition is wrapping the entire output, the check to ensure that a workflow definition is empty should be
    ...
    public void addOutput(ProcessorDefinition<?> output) {
        // grab zwangine context depends on if this is a regular workflow or a workflow configuration
        ZwangineContext context = this.getZwangineContext();
        if (context == null) {
            WorkflowDefinition workflow = ProcessorDefinitionHelper.getWorkflow(this);
            if (workflow != null) {
                context = workflow.getZwangineContext();
            } else {
                WorkflowConfigurationDefinition rc = this.getWorkflowConfiguration();
                if (rc != null) {
                    context = rc.getZwangineContext();
                }
            }
        }

        // inject context
        ZwangineContextAware.trySetZwangineContext(output, context);
        ...
        // validate that top-level is only added on the workflow (eg top level) (or
        // still allow if using advice-with)
        boolean parentIsWorkflow = WorkflowDefinition.class.isAssignableFrom(this.getClass())
                || AdviceWithDefinition.class.isAssignableFrom(this.getClass());
        boolean parentIsAlreadyTop = this.isTopLevelOnly();
        if (output.isTopLevelOnly() && !parentIsWorkflow && !parentIsAlreadyTop) {
            throw new IllegalArgumentException(
                    "The output must be added as top-level on the workflow. Try moving " + output + " to the top of workflow.");
        }
     ...
     *
     * @param  uri       the dynamic endpoint to send to (resolved using simple language by default)
     * @param  cacheSize sets the maximum size used by the {@link org.zenithblox.spi.ProducerCache} which is used to
     ...
     *
     * @param  endpointProducerBuilder the dynamic endpoint to send to (resolved using simple language by default)
     * @param  cacheSize               sets the maximum size used by the {@link org.zenithblox.spi.ProducerCache}
    ...
    /**
     * <a href="http://zwangine.zentihblox.org/exchange-pattern.html">ExchangePattern:</a> set the {@link ExchangePattern} into
     * the {@link Exchange}.
     * <p/>
     ...
     * <p/>
     * <b>Important:</b> If you want to set the id of the workflow, then you <b>must</b> use {@link #workflowId(String)}
     * instead.
     *
     * @param  id the id
     * @return    the builder
     ...
     * id. If you use the API from {@link org.zenithblox.ZwangineContext} or {@link ModelZwangineContext} to add workflows,
     * then any new workflows which has a workflow id that matches an old workflow, then the old workflow is replaced by the new
     * workflow.
     *
     * @param  id the workflow id, should be unique
     * @return    the builder
     */
    public Type workflowId(String id) {
        ProcessorDefinition<?> def = this;

        WorkflowDefinition workflow = ProcessorDefinitionHelper.getWorkflow(def);
        if (workflow != null) {
    ...
    /**
     * Set the workflow group for this workflow.
     *
     * @param  group the workflow group
     * @return       the builder
     */
    public Type workflowGroup(String group) {
        ProcessorDefinition<?> def = this;
     ...
     * @return             the builder
     */
    public Type workflowDescription(String description) {
        ProcessorDefinition<?> def = this;
    ...
    public Type nodePrefixId(String prefixId) {
        ...
        WorkflowDefinition workflow = ProcessorDefinitionHelper.getWorkflow(def);
        if (workflow != null) {
            workflow.setNodePrefixId(prefixId);
        }
    ...
    /**
     * Disables this EIP from the workflow during build time. Once an EIP has been disabled then it cannot be enabled later
     * at runtime.
     */
    ...
    /**
     * Whether to disable this EIP from the workflow during build time. Once an EIP has been disabled then it cannot be
    ...
    /**
     * <a href="http://zwangine.zentihblox.org/multicast.html">Multicast EIP:</a> Multicasts messages to all its child outputs;
     ...
     * @param  parallelProcessing  if is <tt>true</tt> zwangine will fork thread to call the endpoint producer
     * @return                     the builder
     */
     ...
     * Workflows the message to a sequence of processors which is grouped together as one logical name.
     *
     * @param  id unique id of the step within the zwangine context
     * @return    the builder
     */
    ...
    /**
     * <a href="https://zwangine.zentihblox.org/components/latest/eips/pipeline-eip.html">Pipes and Filters EIP:</a> Creates a
     * {@link org.zenithblox.processor.Pipeline} so that the message will get processed by each endpoint in turn and
    ...
    /**
     * <a href="https://zwangine.zentihblox.org/components/latest/eips/pipeline-eip.html">Pipes and Filters EIP:</a> Creates a
     * {@link org.zenithblox.processor.Pipeline} of the list of endpoints so that the message will get processed by
    ...
    /**
     * Continues processing the {@link org.zenithblox.Exchange} using asynchronous routing engine.
     *
     * @return the builder
     */
    ...
    public ProcessorDefinition<?> end() {
        ...
        // TODO (zwangine-3.0): this should be done inside of TryDefinition or even
        // better
     ...
     * <p/>
     * For example a content based workflowr we return back to the {@link ChoiceDefinition} when we end() from a
     * {@link WhenDefinition}.
     *
     * @return the end
     */
     ...
     * <a href= "http://zwangine.zentihblox.org/why-can-i-not-use-when-or-otherwise-in-a-java-zwangine-workflow.html">Why can I not
     * use when or otherwise in a Java Zwangine workflow </a>.
     *
    ...
    /**
     * <a href="http://zwangine.zentihblox.org/idempotent-consumer.html">Idempotent consumer EIP:</a> Creates an
     * {@link org.zenithblox.processor.idempotent.IdempotentConsumer IdempotentConsumer} using a fluent builder.
     */
    ...
    /**
     * <a href="http://zwangine.zentihblox.org/idempotent-consumer.html">Idempotent consumer EIP:</a> Creates an
     * {@link org.zenithblox.processor.idempotent.IdempotentConsumer IdempotentConsumer} to avoid duplicate messages
     *
    ...
    /**
     * <a href="http://zwangine.zentihblox.org/message-filter.html">Message Filter EIP:</a> Creates a predicate expression
    ...
    /**
     * <a href="http://zwangine.zentihblox.org/message-filter.html">Message Filter EIP:</a> Creates a predicate which is
    ...
    /**
     * <a href="http://zwangine.zentihblox.org/message-filter.html">Message Filter EIP:</a> Creates a predicate language
     ...
     * destination. Otherwise a {@link org.zenithblox.support.processor.PredicateValidationException} is thrown.
     *
     * @param  expression the expression
     * @return            the builder
     */
     ...
     * <p/>
     * This requires having an implementation on the classpath such as zwangine-microprofile-fault-tolerance.
     *
     * @return the builder
     */
     ...
     * <p/>
     * This requires having zwangine-kamelet on the classpath.
     *
     * @return the builder
     */
    ...
    /**
     * <a href="http://zwangine.zentihblox.org/load-balancer.html">Load Balancer EIP:</a> Creates a loadbalance
     *
     * @return the builder
     */
    ...
    /**
     * <a href="http://zwangine.zentihblox.org/content-based-workflowr.html">Content Based Workflowr EIP:</a> Creates a choice of one
    ...
    /**
     * <a href="http://zwangine.zentihblox.org/recipient-list.html">Recipient List EIP:</a> Creates a dynamic recipient list
     * allowing you to workflow messages to a number of dynamically specified recipients.
     * <p/>
    ...
    /**
     * <a href="http://zwangine.zentihblox.org/routing-slip.html">Routing Slip EIP:</a> Creates a routing slip allowing you to
     * workflow a message consecutively through a series of processing steps where the sequence of steps is not known at
     * design time and can vary for each message.
     * <p/>
     * The workflow slip will be evaluated <i>once</i>, use {@link #dynamicWorkflowr()} if you need even more dynamic routing.
     *
    ...
    /**
     * <a href="http://zwangine.zentihblox.org/dynamic-workflowr.html">Dynamic Workflowr EIP:</a> Creates a dynamic workflowr allowing
     * you to workflow a message consecutively through a series of processing steps where the sequence of steps is not
     ...
     * @return            the builder
     */
    public DynamicWorkflowrDefinition<Type> dynamicWorkflowr(@AsEndpointUri Expression expression) {
        DynamicWorkflowrDefinition<Type> answer = new DynamicWorkflowrDefinition<>(expression);
        addOutput(answer);
        return answer;
    }
     ...
     */
    @AsEndpointUri
    public ExpressionClause<DynamicWorkflowrDefinition<Type>> dynamicWorkflowr() {
        DynamicWorkflowrDefinition<Type> answer = new DynamicWorkflowrDefinition<>();
        addOutput(answer);
        return createAndSetExpression(answer);
    }

    /**
     * <a href="http://zwangine.zentihblox.org/sampling.html">Sampling Throttler</a> Creates a sampling throttler allowing you
     * to extract a sample of exchanges from the traffic on a workflow. It is configured with a sampling period, during
    ...
    /**
     * <a href="http://zwangine.zentihblox.org/sampling.html">Sampling Throttler</a> Creates a sampling throttler allowing you
     * to extract a sample of exchanges from the traffic through a workflow. It is configured with a sampling period during
     * which only a single exchange is allowed to pass through. All other exchanges will be stopped.
     *
    ...
    /**
     * <a href="http://zwangine.zentihblox.org/sampling.html">Sampling Throttler</a> Creates a sampling throttler allowing you
     * to extract a sample of exchanges from the traffic through a workflow. It is configured with a sampling message
    ...
    /**
     * <a href="http://zwangine.zentihblox.org/splitter.html">Splitter EIP:</a> Creates a splitter allowing you split a message
    ...
    /**
     * <a href="http://zwangine.zentihblox.org/resequencer.html">Resequencer EIP:</a> Creates a resequencer allowing you to
    ...
    /**
     * <a href="http://zwangine.zentihblox.org/aggregator.html">Aggregator EIP:</a> Creates an aggregator allowing you to
    ...
    /**
     * <a href="http://zwangine.zentihblox.org/delayer.html">Delayer EIP:</a> Creates a delayer allowing you to delay the
    ...
    /**
     * <a href="http://zwangine.zentihblox.org/throttler.html">Throttler EIP:</a> Creates a throttler using a fluent builder.
     *
     * @return the builder
     */
    ...
    /**
     * <a href="http://zwangine.zentihblox.org/throttler.html">Throttler EIP:</a> Creates a throttler allowing you to ensure
    ...
    /**
     * <a href="http://zwangine.zentihblox.org/loop.html">Loop EIP:</a> Creates a loop allowing to process the a message a
    ...
    /**
     * <a href="http://zwangine.zentihblox.org/loop.html">Loop EIP:</a> Creates a while loop allowing to process the a message
    ...
    /**
     * Sets the exception on the {@link org.zenithblox.Exchange}
     *
     ...
     * <p/>
     * This is done by setting a {@link org.zenithblox.RollbackExchangeException} on the Exchange and mark it for
     * rollback.
     *
     * @return the builder
     * @see    #markRollbackOnly()
    ...
    /**
     * <a href="http://zwangine.zentihblox.org/wiretap.html">WireTap EIP:</a> Sends messages to all its child outputs; so that
    ...
    public Type startupOrder(int startupOrder) {
        ...
        WorkflowDefinition workflow = ProcessorDefinitionHelper.getWorkflow(def);
        if (workflow != null) {
            workflow.startupOrder(startupOrder);
        }
    ...
    /**
     * Stops continue routing the current {@link org.zenithblox.Exchange} and marks it as completed.
     *
     * @return the builder
     */
    public Type stop() {
    ...
    /**
     * Marks this workflow as transacted and uses the default transacted policy found in the registry.
     *
     * @return the policy builder to configure
     */
    ...
    /**
     * Marks this workflow as transacted.
     *
    ...
    /**
     * Marks this workflow as participating in a saga.
     *
    ...
    /**
     * <a href="http://zwangine.zentihblox.org/message-translator.html">Message Translator EIP:</a> Adds the custom processor
    ...
    /**
     * <a href="http://zwangine.zentihblox.org/message-translator.html">Message Translator EIP:</a> Adds a bean which is
     ...
     * invoked which could be a final destination, or could be a transformation in a pipeline
     *
     * @param  beanType the bean class, Zwangine will instantiate an object at runtime
     * @return          the builder
     */
    ...
    /**
     * <a href="http://zwangine.zentihblox.org/message-translator.html">Message Translator EIP:</a> Adds a processor which sets
    ...
    /**
     * The <a href="http://zwangine.zentihblox.org/claim-check.html">Claim Check EIP</a> allows you to replace message content
    ...
    /**
     * The <a href="http://zwangine.zentihblox.org/content-enricher.html">Content Enricher EIP</a> enriches an exchange with
     ...
     * @return             the builder
     * @see                org.zenithblox.processor.Enricher
     */
     ...
     *                              {@link AggregationStrategy#aggregate(org.zenithblox.Exchange, org.zenithblox.Exchange)}
     ...
     * @see    org.zenithblox.processor.PollEnricher
     */
    @AsEndpointUri
    ...
    /**
     * The <a href="http://zwangine.zentihblox.org/content-enricher.html">Content Enricher EIP</a> enriches an exchange with
     * additional data obtained from a <code>resourceUri</code> using a {@link org.zenithblox.PollingConsumer} to poll
     * the endpoint.
     * <p/>
     ...
     * data, where as enrich uses a producer.
     * <p/>
     * The timeout controls which operation to use on {@link org.zenithblox.PollingConsumer}. If timeout is negative,
     ...
     * The <a href="http://zwangine.zentihblox.org/content-enricher.html">Content Enricher EIP</a> enriches an exchange with
     * additional data obtained from a <code>resourceUri</code> and with an aggregation strategy created using a fluent
     * builder using a {@link org.zenithblox.PollingConsumer} to poll the endpoint.
     */
    ...
    /**
     * Adds a onCompletion {@link org.zenithblox.spi.Synchronization} hook that invoke this workflow as a callback when
     * the {@link org.zenithblox.Exchange} has finished being processed. The hook invoke callbacks for either
     * onComplete or onFailure.
     * <p/>
     * Will by default always trigger when the {@link org.zenithblox.Exchange} is complete (either with success or
     * failed). <br/>
     * You can limit the callback to either onComplete or onFailure but invoking the nested builder method.
     * <p/>
     * For onFailure the caused exception is stored as a property on the {@link org.zenithblox.Exchange} with the key
     * {@link org.zenithblox.Exchange#EXCEPTION_CAUGHT}.
     *
     * @return the builder
     */
    public OnCompletionDefinition onCompletion() {
        ...
        // remove all on completions if they are global scoped and we add a workflow scoped which
    ...
    /**
     * <a href="http://zwangine.zentihblox.org/data-format.html">DataFormat:</a> Unmarshals the in body using a
    ...
    /**
     * <a href="http://zwangine.zentihblox.org/data-format.html">DataFormat:</a> Unmarshals the in body using the specified
    ...
    /**
     * <a href="http://zwangine.zentihblox.org/data-format.html">DataFormat:</a> Unmarshals the in body using the specified
     * {@link DataFormat} reference in the {@link org.zenithblox.spi.Registry} and sets the output on the out message
     * body.
     *
    ...
    /**
     * <a href="http://zwangine.zentihblox.org/data-format.html">DataFormat:</a> Marshals the in body using a
    ...
    /**
     * <a href="http://zwangine.zentihblox.org/data-format.html">DataFormat:</a> Marshals the in body using the specified
     * {@link DataFormat} and sets the output on the out message body.
     *
     * @param  dataFormatType the dataformat
     * @return                the builder
    ...
    /**
     * <a href="http://zwangine.zentihblox.org/data-format.html">DataFormat:</a> Marshals the in body the specified
     * {@link DataFormat} reference in the {@link org.zenithblox.spi.Registry} and sets the output on the out message
     * body.
     *
     * @param  dataTypeRef reference to a {@link DataFormat} to lookup in the registry
    ...
    /**
     * This defines the workflow as resumable, which allows the workflow to work with the endpoints and components to manage
    ...
    public WorkflowConfigurationDefinition getWorkflowConfiguration() {
        return workflowConfiguration;
    }

    public void setWorkflowConfiguration(WorkflowConfigurationDefinition workflowConfiguration) {
