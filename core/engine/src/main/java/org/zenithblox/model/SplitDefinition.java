...
public class SplitDefinition extends OutputExpressionNode implements ExecutorServiceAwareDefinition<SplitDefinition> {
     ...
     * message from the Splitter. By default Zwangine will use the original incoming message to the splitter (leave it
     ...
     * single outgoing message from the Splitter. By default Zwangine will use the original incoming message to the
     ...
     * do not know the total size, and therefore the {@link org.zenithblox.Exchange#SPLIT_SIZE} is empty.
     * <p/>
     ...
     * more memory. The total size is provided in the {@link org.zenithblox.Exchange#SPLIT_SIZE} header.
     * <p/>
     * The streaming mode also affects the aggregation behavior. If enabled then Zwangine will process replies
     * out-of-order, e.g. in the order they come back. If disabled, Zwangine will process replies in the same order as the
     * messages was split.
     *
     * @return the builder
     */
     ...
     * which it is difficult for Zwangine to shut down in a graceful manner may continue to run. So use this option with a
     * bit of care.
     *
    ...
    /**
     * Shares the {@link org.zenithblox.spi.UnitOfWork} with the parent and each of the sub messages. Splitter will by
