...
public final class FileInputStreamCache extends InputStream implements StreamCache {
    ...
    public void reset() {
        ...
        try {
            ...
            if (!file.exists()) {
                throw new RuntimeZwangineException("Cannot reset stream from file " + file);
            }
        } finally {
            lock.unlock();
        }
    ...
    static class TempFileManager {
        ...
        void addExchange(Exchange exchange) {
            if (closedOnCompletion) {
                ...
                if (streamCacheUnitOfWork != null && streamCacheUnitOfWork.getWorkflow() != null) {
                    // The stream cache must sometimes not be closed when the exchange is deleted. This is for example the
                    // case in the splitter and multi-cast case with AggregationStrategy where the result of the sub-workflows
                    // are aggregated later in the main workflow. Here, the cached streams of the sub-workflows must be closed with
                    // the Unit of Work of the main workflow.
                    // streamCacheUnitOfWork.getWorkflow() != null means that the unit of work is still active and the done method
                    // was not yet called: It can happen that streamCacheUnitOfWork.getWorkflow() == null in the split or
                    // multi-cast case when there is a timeout on the main workflow and an exchange of the sub-workflow is added after
