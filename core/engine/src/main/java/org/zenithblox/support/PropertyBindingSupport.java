...
import org.zenithblox.PropertyBindingException;
...
import org.zenithblox.util.StringQuoteHelper;
...
import static org.zenithblox.util.StringHelper.startsWithIgnoreCase;

/**
 * A convenient support class for binding String valued properties to an instance which uses a set of conventions:
 * <ul>
 * <li>property placeholders - Keys and values using Zwangines property placeholder will be resolved</li>
 ...
 *
 * <p>
 * Keys with dash style is supported and will internally be converted from dash to zwangine case style (eg
...
public final class PropertyBindingSupport {
     ...
     * @see                 #build()
     */
    public static boolean bindProperties(ZwangineContext zwangineContext, Object target, Map<String, Object> properties) {
        // mandatory parameters
        org.zenithblox.util.ObjectHelper.notNull(zwangineContext, "zwangineContext");
        org.zenithblox.util.ObjectHelper.notNull(target, "target");
        org.zenithblox.util.ObjectHelper.notNull(properties, "properties");

        return PropertyBindingSupport.build().bind(zwangineContext, target, properties);
    }
     ...
     * @see                 #build()
     */
    public static boolean bindWithFlattenProperties(ZwangineContext zwangineContext, Object target, Map<String, Object> properties) {
        // mandatory parameters
        org.zenithblox.util.ObjectHelper.notNull(zwangineContext, "zwangineContext");
        org.zenithblox.util.ObjectHelper.notNull(target, "target");
        org.zenithblox.util.ObjectHelper.notNull(properties, "properties");

        return PropertyBindingSupport.build().withFlattenProperties(true).bind(zwangineContext, target, properties);
    }
     ...
     */
    public static void setPropertiesOnTarget(ZwangineContext context, Object target, Map<String, Object> properties) {
        org.zenithblox.util.ObjectHelper.notNull(context, "context");
        org.zenithblox.util.ObjectHelper.notNull(target, "target");
        org.zenithblox.util.ObjectHelper.notNull(properties, "properties");

        if (target instanceof ZwangineContext) {
            throw new UnsupportedOperationException("Configuring the Zwangine Context is not supported");
        }
     ...
     * @param  deepNesting        whether deep nesting is in use, where Zwangine will attempt to walk as deep as possible
     ...
     * @param  placeholder        whether to use Zwangines property placeholder to resolve placeholders on keys and values
    ...
    private static boolean doBindProperties(
            ZwangineContext zwangineContext, Object target, Map<String, Object> properties,
        ...
        if (listener == null && zwangineContext != null) {
            listener = zwangineContext.getRegistry().findSingleByType(PropertyBindingListener.class);
        }
        ...
        for (Map.Entry<String, Object> entry : sorted.entrySet()) {
            ...
            boolean hit = doBuildPropertyOgnlPath(zwangineContext, target, key, value, deepNesting, fluentBuilder,
    ...
    private static boolean doBuildPropertyOgnlPath(
            final ZwangineContext zwangineContext, final Object originalTarget, String name, final Object value,
        ...
        if (configurer == null) {
            ...
            configurer = PropertyConfigurerHelper.resolvePropertyConfigurer(zwangineContext, newClass);
        }
        ...
        for (int i = 0; i < parts.length - 1; i++) {
            ...
            if (configurer != null) {
                prop = getOrCreatePropertyOgnlPathViaConfigurer(zwangineContext, newTarget, part, ignoreCase, configurer);
            }
            if (prop == null && reflection) {
                // no configurer or not possible with configurer so fallback and use reflection
                prop = getOrCreatePropertyOgnlPathViaReflection(zwangineContext, newTarget, part, ignoreCase);
            }
            if (prop == null) {
                if (!deepNesting) {
                ...
                if (configurer != null) {
                    prop = attemptCreateNewInstanceViaConfigurer(zwangineContext, newTarget, part, ignoreCase,
                            configurer);
                }
                if (prop == null && reflection) {
                    // no configurer or not possible with configurer so fallback and use reflection
                    prop = attemptCreateNewInstanceViaReflection(zwangineContext, newTarget, newClass, part, fluentBuilder,
                            allowPrivateSetter,
                            ignoreCase);
                }
            }
            if (prop == null) {
                ...
                if (configurer instanceof PropertyConfigurerGetter propertyConfigurerGetter) {
                    ...
                    configurer = PropertyConfigurerHelper.resolvePropertyConfigurer(zwangineContext, collectionType);
                    if (configurer == null) {
                ...
                if (configurer == null) {
                    configurer = PropertyConfigurerHelper.resolvePropertyConfigurer(zwangineContext, newTarget);
                }
            }
        }
        ...
        return doSetPropertyValue(zwangineContext, newTarget, newName, value, ignoreCase, mandatory,
    ...
    private static Object attemptCreateNewInstanceViaReflection(
            ZwangineContext zwangineContext, Object newTarget, Class<?> newClass, String name, boolean fluentBuilder,
        ...
        Object answer = null;
        Method method = findBestSetterMethod(zwangineContext, newClass, key, fluentBuilder, allowPrivateSetter, ignoreCase);
        if (method != null) {
            Class<?> parameterType = method.getParameterTypes()[0];
            Object obj = getObjectForType(zwangineContext, parameterType);

            if (obj != null) {
                org.zenithblox.support.ObjectHelper.invokeMethod(method, newTarget, obj);
                answer = obj;
            }
        }
        return answer;
    ...
    private static Object attemptCreateNewInstanceViaConfigurer(
            ZwangineContext zwangineContext, Object newTarget, String name,
        ...
        if (parameterType != null) {
            Object obj = getObjectForType(zwangineContext, parameterType);
            if (obj != null) {
                boolean hit = configurer.configure(zwangineContext, newTarget, undashKey(key), obj, ignoreCase);
                if (hit) {
                    answer = obj;
                }
            }
    ...
    private static Object getObjectForType(ZwangineContext zwangineContext, Class<?> parameterType) {
        // special for properties/map/list/array
        Object obj = getObjectForCollectionType(parameterType);

        if (obj == null && org.zenithblox.util.ObjectHelper.hasDefaultPublicNoArgConstructor(parameterType)) {
            obj = zwangineContext.getInjector().newInstance(parameterType);
        }
        return obj;
    }

    private static boolean doSetPropertyValue(
            ZwangineContext zwangineContext, Object target, String name, Object value,
        ...
        if (placeholder) {
            ...
            key = zwangineContext.resolvePropertyPlaceholders(key);
            if (text instanceof String s) {
                // resolve property placeholders
                text = zwangineContext.resolvePropertyPlaceholders(s);
        ...
        try {
            Object str = resolveValue(zwangineContext, target, key, text, ignoreCase, fluentBuilder,
                    allowPrivateSetter, reflection, configurer);
            // resolve property placeholders
            if (str instanceof String strValue) {
                // resolve property placeholders
                str = zwangineContext.resolvePropertyPlaceholders(strValue);
            }
        ...
        try {
            if (isCollectionKey(name)) {
                ...
                if (configurer != null) {
                    bound = setPropertyCollectionViaConfigurer(zwangineContext, target, key, value, ignoreCase, configurer);
                }
                if (!bound && reflection) {
                    // fallback to reflection based
                    bound = setPropertyCollectionViaReflection(zwangineContext, target, key, value, ignoreCase, reference,
                            optional);
                }
            } else {
                // regular key
                if (configurer != null) {
                    bound = setSimplePropertyViaConfigurer(zwangineContext, target, key, value, ignoreCase, configurer);
                }
                if (!bound && reflection) {
                    // fallback to reflection based
                    bound = setSimplePropertyViaReflection(zwangineContext, target, key, value, fluentBuilder, allowPrivateSetter,
                ...
                if (!bound && target instanceof List && StringHelper.isDigit(key)) {
                    try {
                        ...
                        org.zenithblox.util.ObjectHelper.addListByIndex((List) target, idx, value);
                        bound = true;
    ...
    private static boolean setPropertyCollectionViaReflection(
            ZwangineContext context, Object target, String name, Object value,
        ...
        if (value instanceof String str) {
            if (reference && isReferenceParameter(str)) {
                Object bean = ZwangineContextHelper.lookup(context, str.substring(1));
                if (bean != null) {
                    value = bean;
                }
            }
        ...
        if (obj instanceof Map) {
            ...
            if (isNotEmpty(lookupKey)) {
                ...
                org.zenithblox.util.ObjectHelper.addListByIndex(list, idx, value);
            } else {
                list.add(value);
            }
            return true;
    ...
    private static boolean setPropertyCollectionViaConfigurer(
        ...
        if (obj == null) {
            ...
            if (obj != null) {
                // set
                boolean hit = configurer.configure(zwangineContext, target, undashKey, obj, ignoreCase);
                if (!hit) {
                    // not a map or list
                    throw new IllegalArgumentException(
                            "Cannot set property: " + name
        ...
        if (obj instanceof Map) {
            ...
            if (idx >= size) {
                obj = Arrays.copyOf((Object[]) obj, idx + 1);
                // replace array
                boolean hit = configurer.configure(zwangineContext, originalTarget, undashKey, obj, ignoreCase);
                if (!hit) {
                    throw new IllegalArgumentException(
                            "Cannot set property: " + name
                                                       + " as an array because target bean has no setter method for the array");
    ...
    private static boolean setSimplePropertyViaConfigurer(
            ZwangineContext zwangineContext, Object target, String key, Object value,
            boolean ignoreCase, PropertyConfigurer configurer) {
        try {
            return configurer.configure(zwangineContext, target, undashKey(key), value, ignoreCase);
        } catch (Exception e) {
            throw new PropertyBindingException(target, key, value, e);
        }
    }
    ...
    private static boolean setSimplePropertyViaReflection(
        ...
        try {
            if (name != null) {
                return doSetSimplePropertyViaReflection(zwangineContext, target, name, value, false, ignoreCase, fluentBuilder,
    ...
    private static boolean doSetSimplePropertyViaReflection(
            ZwangineContext context, Object target, String name, Object value, boolean mandatory,
    ...
    private static Object getOrCreatePropertyOgnlPathViaConfigurer(
            ZwangineContext context, Object target, String property, boolean ignoreCase, PropertyConfigurer configurer) {
        String key = property;
        ...
        if (answer instanceof Map && lookupKey != null) {
            ...
            if (answer == null) {
                ...
                parameterType = (Class<?>) ((PropertyConfigurerGetter) configurer).getCollectionValueType(target, undashKey,
                        ignoreCase);
                if (parameterType != null
                        && org.zenithblox.util.ObjectHelper.hasDefaultPublicNoArgConstructor(parameterType)) {
    ...
    private static Object getOrCreatePropertyOgnlPathViaReflection(
            ZwangineContext context, Object target, String property, boolean ignoreCase) {
        String key = property;
        String lookupKey = null;
    ...
    private static Method findBestSetterMethod(
            ZwangineContext context, Class<?> clazz, String name,
     ...
     */
    public static Object newInstanceConstructorParameters(ZwangineContext zwangineContext, Class<?> type, String parameters)
            throws Exception {
        String[] params = StringQuoteHelper.splitSafeQuote(parameters, ',');
        Constructor<?> found = findMatchingConstructor(zwangineContext, type.getConstructors(), params);
        if (found != null) {
            ...
            for (int i = 0; i < found.getParameterCount(); i++) {
                ...
                if (param instanceof String str) {
                    if (str.startsWith("#")) {
                        Object bean = resolveBean(zwangineContext, param);
                        if (bean != null) {
                            val = bean;
                        }
                    }
                ...
                if (val != null) {
                    val = zwangineContext.getTypeConverter().tryConvertTo(paramType, val);
                } else {
                    val = zwangineContext.getTypeConverter().convertTo(paramType, param);
                }
                arr[i] = val;
            }
     ...
     * <p/>
     * This implementation is similar to the logic in zwangine-bean.
     *
    ...
    private static Constructor<?> findMatchingConstructor(
            ZwangineContext zwangineContext, Constructor<?>[] constructors, String[] params) {
        ...
        for (Constructor<?> ctr : constructors) {
            ...
            for (int i = 0; i < ctr.getParameterCount(); i++) {
                ...
                Class<?> parameterType = getValidParameterType(zwangineContext, parameter);
    ...
    public static Object newInstanceFactoryParameters(
            ZwangineContext zwangineContext, Class<?> type, String factoryMethod, String parameters)
            throws Exception {
        String[] params = StringQuoteHelper.splitSafeQuote(parameters, ',');
        Method found = findMatchingFactoryMethod(zwangineContext, type.getMethods(), factoryMethod, params);
        if (found != null) {
            Object[] arr = new Object[found.getParameterCount()];
            for (int i = 0; i < found.getParameterCount(); i++) {
                Class<?> paramType = found.getParameterTypes()[i];
    ...
    private static Method findMatchingFactoryMethod(
            ZwangineContext zwangineContext, Method[] methods, String factoryMethod, String[] params) {
     ...
     */
    private static Class<?> getValidParameterType(ZwangineContext zwangineContext, String value) {
        if (org.zenithblox.util.ObjectHelper.isEmpty(value)) {
            return null;
        }
        ...
        if (value.startsWith("#")) {
            ...
            Object bean = ZwangineContextHelper.lookup(zwangineContext, value);
            if (bean != null) {
     ...
     */
    public static Object resolveBean(ZwangineContext zwangineContext, Object value) throws Exception {
        ...
        strval = zwangineContext.resolvePropertyPlaceholders(strval);
        ...
        if (strval.startsWith("#class:")) {
            ...
            Class<?> type = zwangineContext.getClassResolver().resolveMandatoryClass(className);
            if (factoryMethod != null) {
                ...
                if (parameters != null) {
                    ...
                    answer = newInstanceFactoryParameters(zwangineContext, target, factoryMethod, parameters);
                } else {
                ...
                answer = newInstanceConstructorParameters(zwangineContext, type, parameters);
            } else {
                answer = zwangineContext.getInjector().newInstance(type);
            }
            if (answer == null) {
            ...
            Class<?> type = zwangineContext.getClassResolver().resolveMandatoryClass(typeName);
            answer = zwangineContext.getRegistry().mandatoryFindSingleByType(type);
        } else if (strval.startsWith("#bean:")) {
            String key = strval.substring(6);
            answer = ZwangineContextHelper.mandatoryLookup(zwangineContext, key);
            ...
            Class<?> type = zwangineContext.getClassResolver().resolveMandatoryClass(typeName);
            answer = zwangineContext.getTypeConverter().mandatoryConvertTo(type, constant);
        }
    ...
    public static class Builder {
        ...
        /**
         * ZwangineContext to be used
         */
        public Builder withZwangineContext(ZwangineContext zwangineContext) {
        ...
        /**
         * Whether deep nesting is in use, where Zwangine will attempt to walk as deep as possible by creating new objects
        ...
        /**
         * Whether to use Zwangines property placeholder to resolve placeholders on keys and values
         */
        ...
        public <T> T build(Class<T> type, String buildMethod) {
            ...
            try {
                ...
                return zwangineContext.getTypeConverter().convertTo(type, out);
            } catch (Exception e) {
        ...
        public boolean bind() {
            ...
            return doBindProperties(zwangineContext, target, removeParameters ? properties : new HashMap<>(properties),
         ...
         * @return              true if one or more properties was bound
         */
        public boolean bind(ZwangineContext zwangineContext, Object target, Map<String, Object> properties) {
            ZwangineContext context = zwangineContext != null ? zwangineContext : this.zwangineContext;
         ...
         */
        public boolean bind(ZwangineContext zwangineContext, Object target, String key, Object value) {
            Map<String, Object> properties = new HashMap<>(1);
            properties.put(key, value);

            return doBindProperties(zwangineContext, target, properties, optionPrefix, ignoreCase, true, false, mandatory,
                    optional,
                    nesting, deepNesting, fluentBuilder, allowPrivateSetter, reference, placeholder, reflection, configurer,
                    listener);
        }
    ...
    private static final class MapConfigurer implements PropertyConfigurer {
        ...
        public boolean configure(
                ZwangineContext zwangineContext, Object target, String name, Object value, boolean ignoreCase) {
