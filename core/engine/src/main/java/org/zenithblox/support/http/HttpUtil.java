...
public final class HttpUtil {
     ...
     *
     * @param  zwangineExchange the exchange to evaluate
     ...
     */
    public static int determineResponseCode(Exchange zwangineExchange, Object body) {
        boolean failed = zwangineExchange.isFailed();
        int defaultCode = failed ? INTERNAL_SERVER_ERROR : OK;

        Message message = zwangineExchange.getMessage();
    ...
    public static void applyHeader(
        ...
        while (it.hasNext()) {
            ...
            if (headerValue != null && !headerFilterStrategy.applyFilterToZwangineHeaders(key, headerValue, exchange)) {
                if (firstValue == null) {
     ...
     * if there is a match to an existing Zwangine Message header, then we should filter (=true) this
     * header as its already been mapped by the RestProducer from zwangine-core, and we do not want
