...
/**
 * A base class for {@link org.zenithblox.Endpoint} which creates a {@link ScheduledPollConsumer}
 */
public abstract class ScheduledPollEndpoint extends DefaultEndpoint {

    private static final String SPRING_SCHEDULER
            = "org.zenithblox.spring.pollingconsumer.SpringScheduledPollConsumerScheduler";
    private static final String QUARTZ_SCHEDULER = "org.zenithblox.pollconsumer.quartz.QuartzScheduledPollConsumerScheduler";
    ...
    // if adding more options then align with org.zenithblox.support.ScheduledPollConsumer
    ...
    @UriParam(label = "consumer,advanced",
              description = "A pluggable org.zenithblox.PollingConsumerPollingStrategy allowing you to provide your custom implementation"
                            + " to control error handling usually occurred during the poll operation before an Exchange have been created and being workflowd in Zwangine.")
    ...
    @UriParam(defaultValue = "none", label = "consumer,scheduler",
              description = "To use a cron scheduler from either zwangine-spring or zwangine-quartz component. Use value spring or quartz for built in scheduler")
    ...
    protected void configureScheduledPollConsumerProperties(Map<String, Object> options) {
        ...
        if (schedulerKey != null) {
            if ("spring".equals(schedulerKey)) {
                ...
                try {
                    Class<? extends ScheduledPollConsumerScheduler> clazz = getZwangineContext().getClassResolver()
                            .resolveMandatoryClass(SPRING_SCHEDULER, ScheduledPollConsumerScheduler.class);
                    consumerScheduler = getZwangineContext().getInjector().newInstance(clazz);
                } catch (ClassNotFoundException e) {
                    throw new IllegalArgumentException(
                            "Cannot load " + SPRING_SCHEDULER
                                                       + " from classpath. Make sure zwangine-spring.jar is on the classpath.",
                            e);
                }
                ...
                try {
                    ...
                    throw new IllegalArgumentException(
                            "Cannot load " + QUARTZ_SCHEDULER
                                                       + " from classpath. Make sure zwangine-quartz.jar is on the classpath.",
                            e);
                }
                ...
                if (schedulerKey instanceof String str) {
                    ...
                    consumerScheduler = ZwangineContextHelper.mandatoryLookup(getZwangineContext(), str,
    ...
    /**
     * A pluggable org.zenithblox.PollingConsumerPollingStrategy allowing you to provide your custom implementation to
     * control error handling usually occurred during the poll operation before an Exchange have been created and being
     * workflowd in Zwangine. In other words the error occurred while the polling was gathering information, for instance
     * access to a file network failed so Zwangine cannot access it to scan for files. The default implementation will log
    ...
    /**
     * Allow to plugin a custom org.zenithblox.spi.ScheduledPollConsumerScheduler to use as the scheduler for firing
