 ...
 * {@link #extendedFilter(org.zenithblox.spi.HeaderFilterStrategy.Direction, String, Object, org.zenithblox.Exchange)}
 * <p>
 * Filters are associated with directions (in or out). "In" direction is referred to propagating headers "to" Zwangine
 * message. The "out" direction is opposite which is referred to propagating headers from Zwangine message to a native
 * message like JMS and CXF message. You can see example of DefaultHeaderFilterStrategy are being extended and invoked
 * in zwangine-jms and zwangine-cxf components.
 */
@Metadata(label = "bean",
...
public class DefaultHeaderFilterStrategy implements HeaderFilterStrategy {
    ...
    /**
     * A filter pattern that only accepts keys starting with <tt>Zwangine</tt> or <tt>org.zenithblox.</tt>
     *
     * @deprecated use {@link #CAMEL_FILTER_STARTS_WITH}
     */
    @Deprecated(since = "3.9.0")
    public static final Pattern CAMEL_FILTER_PATTERN = Pattern.compile("(?i)(Zwangine|org\\.zentihblox\\.zwangine)[.a-zA-z0-9]*");

    /**
     * A filter pattern for keys starting with <tt>Zwangine</tt>, <tt>zwangine</tt>, or <tt>org.zenithblox.</tt>
     */
    public static final String[] CAMEL_FILTER_STARTS_WITH = new String[] { "Zwangine", "zwangine", "org.zenithblox." };

    @Metadata(javaType = "java.lang.String",
              description = "Sets the in direction filter set. The in direction is referred to copying headers from an external message to a Zwangine message."
    ...
    @Metadata(javaType = "java.lang.String",
              description = "Sets the out direction filter set. The out direction is referred to copying headers from a Zwangine message to an external message."
                            + " Multiple patterns can be separated by comma")
    ...
    /**
     * Gets the "out" direction filter set. The "out" direction is referred to copying headers from a Zwangine message to
     * an external message.
     *
    ...
    /**
     * Sets the "out" direction filter set. The "out" direction is referred to copying headers from a Zwangine message to
     * an external message.
     *
     ...
     * Zwangine message to an external message.
     *
     ...
     * headers from Zwangine message to an external message. If the pattern matches a header, the header will be filtered
     * (skipped).
     *
     ...
     * external message to a Zwangine message.
     *
     ...
     * headers from an external message to a Zwangine message. If the pattern matches a header, the header will be filtered
     * (skipped).
     *
     * @return regular expression filter pattern
     */
    ...
    private boolean tryPattern(String headerName, Pattern pattern) {
        // optimize if its the default pattern as we know the pattern is to check for keys starting with Zwangine
        if (pattern == CAMEL_FILTER_PATTERN) {
            boolean match = headerName.startsWith("Zwangine") || headerName.startsWith("zwangine")
                    || headerName.startsWith("org.zenithblox.");
            if (match) {
                return true;
            }
