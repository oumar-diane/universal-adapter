...
import org.zenithblox.spi.RestClientRequestValidator;
...
public class RestBindingAdviceFactory {
     ...
     */
    public static RestBindingAdvice build(ZwangineContext zwangineContext, RestBindingConfiguration bc) throws Exception {
        ...
        if (mode.contains("json") || "auto".equals(mode) || bc.isClientRequestValidation()) {
            ...
            if (name != null) {
                ...
                Object instance = lookupByName(zwangineContext, name);
                if (instance != null) {
            ...
            if (optional) {
                try {
                    json = zwangineContext.createDataFormat(name);
                } catch (IllegalArgumentException e) {
                    // ignore
                }
            } else {
                json = zwangineContext.createDataFormat(name);
                outJson = zwangineContext.createDataFormat(name);
            }

            if (json != null) {
                setupJson(zwangineContext, config,
        ...
        if (mode.contains("xml") || "auto".equals(mode)) {
            ...
            // pre-created
            jaxb = zwangineContext.createDataFormat(name);
            outJaxb = zwangineContext.createDataFormat(name);
            ...
            if (jaxb != null) {
                // to setup JAXB we need to use zwangine-jaxb
                PluginHelper.getRestBindingJaxbDataFormatFactory(zwangineContext).setupJaxb(zwangineContext, config,
                        bc.getType(), bc.getTypeClass(),
                        bc.getOutType(), bc.getOutTypeClass(),
                        jaxb, outJaxb);
            }
        ...
        if (bc.isClientRequestValidation()) {
            validator = lookupRestClientRequestValidator(zwangineContext);
        }

        return new RestBindingAdvice(
                zwangineContext, json, jaxb, outJson, outJaxb,
    ...
    protected static void setupJson(
        ...
        if (typeClass != null) {
            ...
            clazz = zwangineContext.getClassResolver().resolveMandatoryClass(typeName);
        }
        final BeanIntrospection beanIntrospection = PluginHelper.getBeanIntrospection(zwangineContext);
        if (clazz != null) {
            beanIntrospection.setProperty(zwangineContext, json,
                    "unmarshalType", clazz);
            beanIntrospection.setProperty(zwangineContext, json, "useList",
                    useList);
        }
        setAdditionalConfiguration(zwangineContext, config, json, "json.in.");
        ...
        if (outJson != null) {
            ...
            if (outTypeClass != null) {
                ...
                outClazz = zwangineContext.getClassResolver().resolveMandatoryClass(typeName);
            }
            if (outClazz != null) {
                beanIntrospection.setProperty(zwangineContext, outJson,
                        "unmarshalType", outClazz);
                beanIntrospection.setProperty(zwangineContext, outJson, "useList",
                        outUseList);
            }
            setAdditionalConfiguration(zwangineContext, config, outJson, "json.out.");
        }
    }

    protected static RestClientRequestValidator lookupRestClientRequestValidator(ZwangineContext zwangineContext) {
        RestClientRequestValidator answer = ZwangineContextHelper.findSingleByType(zwangineContext, RestClientRequestValidator.class);
        if (answer == null) {
    ...
    private static void setAdditionalConfiguration(
            ZwangineContext zwangineContext, RestConfiguration config, DataFormat dataFormat, String prefix) {
        if (config.getDataFormatProperties() != null && !config.getDataFormatProperties().isEmpty()) {
            ...
            PropertyBindingSupport.build().bind(zwangineContext, dataFormat, copy);
        }
    }
    ...
    private static Object lookupByName(ZwangineContext zwangineContext, String name) {
        if (name == null) {
            return null;
        }
        ...
        if (EndpointHelper.isReferenceParameter(name)) {
            ...
            return zwangineContext.getRegistry().lookupByName(name);
        }
    }
