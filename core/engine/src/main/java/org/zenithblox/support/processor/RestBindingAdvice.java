 ...
 * <p/>
 * The binding uses {@link org.zenithblox.spi.DataFormat} for the actual work to transform from xml/json to Java
 * Objects and reverse again.
 * <p/>
 * The rest producer side is implemented in {@link org.zenithblox.component.rest.RestProducerBindingProcessor}
 *
 * @see RestBindingAdviceFactory
 */
public class RestBindingAdvice extends ServiceSupport implements ZwangineInternalProcessorAdvice<Map<String, Object>> {
     ...
     */
    public RestBindingAdvice(ZwangineContext zwangineContext, DataFormat jsonDataFormat, DataFormat xmlDataFormat,
        ...
        if (jsonMarshal != null) {
            zwangineContext.addService(jsonMarshal, true);
        }
        if (jsonUnmarshal != null) {
            zwangineContext.addService(jsonUnmarshal, true);
        }
        if (xmlMarshal != null) {
            zwangineContext.addService(xmlMarshal, true);
        }
        if (xmlUnmarshal != null) {
            zwangineContext.addService(xmlUnmarshal, true);
        }
    ...
    private boolean isOptionsMethod(Exchange exchange, Map<String, Object> state) {
        ...
        if ("OPTIONS".equalsIgnoreCase(method)) {
            // for OPTIONS methods then we should not workflow at all as its part of CORS
        ...
        if ("off".equals(bindingMode) || bindingMode.equals("auto")) {
            ...
            if (bindingMode.contains("xml")) {
                exchange.setException(
                        new ZwangineExchangeException("Cannot bind to xml as message body is not xml compatible", exchange));
            } else {
                exchange.setException(
                        new ZwangineExchangeException("Cannot bind to json as message body is not json compatible", exchange));
            }
        }
