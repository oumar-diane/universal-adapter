...
/**
 * A default implementation of the {@link org.zenithblox.PollingConsumer} which uses the normal asynchronous consumer
...
public class EventDrivenPollingConsumer extends PollingConsumerSupport implements Processor, IsSingleton {
    ...
    public EventDrivenPollingConsumer(Endpoint endpoint, int queueSize) {
        ...
        this.interruptedExceptionHandler
                = new LoggingExceptionHandler(endpoint.getZwangineContext(), EventDrivenPollingConsumer.class);
    }
    ...
    protected Exchange prepareCopy(Exchange exchange, boolean handover) {
        // use a new copy of the exchange to workflow async (and use same message id)
        ...
        UnitOfWork uow = PluginHelper.getUnitOfWorkFactory(getEndpoint().getZwangineContext()).createUnitOfWork(copy);
