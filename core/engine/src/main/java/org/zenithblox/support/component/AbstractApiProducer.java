...
public abstract class AbstractApiProducer<E extends Enum<E> & ApiName, T>
        ...
        Runnable invocation = new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    exchange.setException(RuntimeZwangineException.wrapRuntimeZwangineException(t));
                } finally {
                    callback.done(false);
                }
            }
     ...
     * @throws RuntimeZwangineException on error. Exceptions thrown by API method are wrapped.
     */
    protected Object doInvokeMethod(ApiMethod method, Map<String, Object> properties) throws RuntimeZwangineException {
            ...
            if (value != null) {
                try {
                    ...
                    if (configurer instanceof PropertyConfigurerGetter getter) {
                        ...
                        if (type != null) {
                            value = endpoint.getZwangineContext().getTypeConverter().mandatoryConvertTo(type, exchange, value);
                        }
                    } else {
                        // fallback to be reflection based
                        value = endpoint.getZwangineContext().getTypeConverter().mandatoryConvertTo(
                                endpoint.getConfiguration().getClass().getDeclaredField(inBodyProperty).getType(),
                                exchange, value);
                    }
                } catch (Exception e) {
                    exchange.setException(new RuntimeZwangineException(
                            String.format(
