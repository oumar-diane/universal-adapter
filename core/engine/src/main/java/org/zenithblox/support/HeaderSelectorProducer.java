...
import org.zenithblox.spi.InvokeOnHeaderStrategy;
 ...
 *
 * @see org.zenithblox.spi.InvokeOnHeader
 * @see InvokeOnHeaderStrategy
 */
public abstract class HeaderSelectorProducer extends DefaultAsyncProducer implements ZwangineContextAware {

    public static final String RESOURCE_PATH = "META-INF/services/org/zentihblox/zwangine/invoke-on-header/";
    ...
    protected void doBuild() throws Exception {
        ...
        strategy = zwangineContext.getZwangineContextExtension().getBootstrapFactoryFinder(RESOURCE_PATH)
        ...
        if (sclazz != null && !sclazz.getName().equals("java.lang.Object")
            ...
            // some components may have a common base class they extend from (such as zwangine-infinispan)
            // so try to discover that (optional so return null if not present)
            String key2 = this.getClass().getSuperclass().getName();
            parentStrategy = zwangineContext.getZwangineContextExtension().getBootstrapFactoryFinder(RESOURCE_PATH)
