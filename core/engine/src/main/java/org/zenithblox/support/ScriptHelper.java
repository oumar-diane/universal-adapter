...
public final class ScriptHelper {
     ...
     */
    public static String resolveOptionalExternalScript(ZwangineContext zwangineContext, String expression) {
        return resolveOptionalExternalScript(zwangineContext, null, expression);
    }
     ...
     * resource could not be loaded.
     */
    public static String resolveOptionalExternalScript(ZwangineContext zwangineContext, Exchange exchange, String expression) {
        if (expression == null) {
            return null;
        }
        ...
        if (hasExternalScript(external)) {
            ...
            try {
                if (optional) {
                    is = ResourceHelper.resolveResourceAsInputStream(zwangineContext, external);
                } else {
                    is = ResourceHelper.resolveMandatoryResourceAsInputStream(zwangineContext, external);
                }
                if (is != null) {
                    expression = zwangineContext.getTypeConverter().convertTo(String.class, is);
                } else {
                    expression = null;
                }
            } catch (IOException e) {
                throw new RuntimeZwangineException("Cannot load resource " + external, e);
            } finally {
                IOHelper.close(is);
            }
        }
