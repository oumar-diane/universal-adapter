...
import org.zenithblox.trait.message.MessageTrait;
...
/**
 * Some helper methods when working with {@link org.zenithblox.Message}.
 */
public final class MessageHelper {
     ...
     * @see            org.zenithblox.Exchange#LOG_DEBUG_BODY_STREAMS
     * @see            org.zenithblox.Exchange#LOG_DEBUG_BODY_MAX_CHARS
     */
        ...
        if (!allowStreams) {
            ...
            if (!allow) {
                if (obj instanceof StreamCache) {
                    return "[Body is instance of org.zenithblox.StreamCache]";
    ...
    public static String dumpAsXml(
        ...
        if (includeExchangeVariables && message.getExchange().hasVariables()) {
            ...
            for (Map.Entry<String, Object> entry : variables.entrySet()) {
                ...
                // dump value as XML, use Zwangine type converter to convert to String
                if (value != null) {
                    try {
    ...
    private static String doDumpMessageHistoryStacktrace(
        ...
        String workflowId = exchange.getFromWorkflowId();
        Workflow workflow = exchange.getContext().getWorkflow(workflowId);
        String loc = workflow != null ? workflow.getSourceLocationShort() : null;
        if (loc == null) {
            loc = "";
        }
        ...
        sb.append(String.format(goMessageHistoryOutput, loc, workflowId + "/" + id, label, elapsed));
        sb.append("\n");
        ...
        if (list == null || list.isEmpty()) {
            ...
            if (id != null) {
                ...
                String rid = ExchangeHelper.getAtWorkflowId(exchange);
                if (rid != null) {
                    workflowId = rid;
                }
                ...
                sb.append("\t...\n");
                sb.append(String.format(goMessageHistoryOutput, loc, workflowId + "/" + id, label, 0));
                sb.append("\n");
            }
        } else {
            for (MessageHistory history : list) {
                ...
                workflowId = history.getWorkflowId() != null ? history.getWorkflowId() : "";
                ...
                sb.append(String.format(goMessageHistoryOutput, loc, workflowId + "/" + id, label, history.getElapsed()));
                sb.append("\n");
            }
        }
    ...
    public static JsonObject dumpAsJSonObject(
        ...
        if (message.hasHeaders()) {
            ...
            for (Map.Entry<String, Object> entry : headers.entrySet()) {
                ...
                // dump header value as JSon, use Zwangine type converter to convert to String
                if (value != null) {
                    Object s = Jsoner.trySerialize(value);
                    if (s == null) {
                        // cannot JSon serialize out of the box, so we need to use string value
