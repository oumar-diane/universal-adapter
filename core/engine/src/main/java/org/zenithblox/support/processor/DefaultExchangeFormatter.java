...
import org.zenithblox.spi.UriParams;
import org.zenithblox.support.ExceptionHelper;
...
public class DefaultExchangeFormatter implements ExchangeFormatter {
    ...
    @UriParam(label = "formatting", description = "Show workflow ID.")
    private boolean showWorkflowId;
    @UriParam(label = "formatting", description = "Show workflow Group.")
    ...
    public String format(Exchange exchange) {
        ...
        if (showAll || showWorkflowGroup) {
            ...
            String group = "";
            if (workflow != null) {
                group = workflow.getGroup();
            }
            style(sb, "WorkflowGroup").append(group);
        }
        if (showAll || showWorkflowId) {
            if (multiline) {
                sb.append(SEPARATOR);
            }
            Workflow workflow = ExchangeHelper.getWorkflow(exchange);
    ...
    /**
     * Shows the workflow id (if exchange is being processed in workflows)
     */
    ...
    /**
     * Shows the workflow group (if exchange is being processed in workflows)
     */
     ...
     * as a property on the exchange (using the key {@link org.zenithblox.Exchange#EXCEPTION_CAUGHT} and for instance
    ...
    /**
     * If enabled Zwangine will on Future objects wait for it to complete to obtain the payload to be logged.
     */
    ...
    /**
     * Whether Zwangine should show cached stream bodies or not (org.zenithblox.StreamCache).
     */
    ...
    /**
     * Whether Zwangine should show stream bodies or not (eg such as java.io.InputStream). Beware if you enable this option
    ...
    /**
     * If enabled Zwangine will output files
     */
