...
import org.zenithblox.language.simple.SimpleExpressionBuilder;
...
import org.zenithblox.spi.SimpleLanguageFunctionFactory;
...
/**
 * Represents one of built-in functions of the <a href="http://zwangine.zentihblox.org/simple.html">simple language</a>
 */
public class SimpleFunctionExpression extends LiteralExpression {
    ...
    /**
     * Creates a Zwangine {@link Expression} based on this model.
     *
    ...
    public Expression createExpression(ZwangineContext zwangineContext, String expression) {
        ...
        if (answer == null) {
            answer = createSimpleExpression(zwangineContext, function, true);
            if (answer != null) {
                answer.init(zwangineContext);
            }
    ...
    private Expression createSimpleExpression(ZwangineContext zwangineContext, String function, boolean strict) {
        // return the function directly if we can create function without analyzing the prefix
        Expression answer = createSimpleExpressionDirectly(zwangineContext, function);
        if (answer != null) {
            return answer;
        }

        // message first
        answer = createSimpleExpressionMessage(zwangineContext, function, strict);
        if (answer != null) {
            return answer;
        }
        ...
        // zwangineContext OGNL
        String remainder = ifStartsWithReturnRemainder("zwangineContext", function);
        if (remainder != null) {
            boolean invalid = OgnlHelper.isInvalidValidOgnlExpression(remainder);
            if (invalid) {
                throw new SimpleParserException("Valid syntax: ${zwangineContext.OGNL} was: " + function, token.getIndex());
            }
            return SimpleExpressionBuilder.zwangineContextOgnlExpression(remainder);
        }
        ...
        if (remainder != null) {
            ...
            Expression inlined = zwangineContext.resolveLanguage("simple").createExpression(exp);
        ...
        if (remainder != null) {
            ...
            if (ref != null && ref.startsWith("type:")) {
                try {
                    type = zwangineContext.getClassResolver().resolveMandatoryClass(ref.substring(5));
                    ref = null;
        ...
        if ("attachments".equals(function) || ifStartsWithReturnRemainder("attachment", function) != null) {
            Expression exp = createSimpleAttachments(zwangineContext, function);
            if (exp != null) {
                return exp;
            }
        }
    ...
    private Expression createSimpleAttachments(ZwangineContext zwangineContext, String function) {
        Optional<SimpleLanguageFunctionFactory> factory = ResolverHelper.resolveService(
                ...
                SimpleLanguageFunctionFactory.FACTORY + "/zwangine-attachments",
                SimpleLanguageFunctionFactory.class);

        if (factory.isEmpty()) {
            throw new IllegalArgumentException(
                    "Cannot find SimpleLanguageFunctionFactory on classpath. Add zwangine-attachments to classpath.");
        }
        return factory.get().createFunction(zwangineContext, function, token.getIndex());
    }

    private Expression createSimpleExpressionMessage(ZwangineContext zwangineContext, String function, boolean strict) {
        // messageAs
    ...
    private Expression createSimpleExpressionDirectly(ZwangineContext zwangineContext, String expression) {
        ...
        } else if (ObjectHelper.equal(expression, "zwangineId")) {
            return ExpressionBuilder.zwangineContextNameExpression();
        } else if (ObjectHelper.equal(expression, "routeId")) {
            return ExpressionBuilder.workflowIdExpression();
        } else if (ObjectHelper.equal(expression, "fromWorkflowId")) {
            return ExpressionBuilder.fromWorkflowIdExpression();
        } else if (ObjectHelper.equal(expression, "routeGroup")) {
            return ExpressionBuilder.workflowGroupExpression();
    ...
    private String doCreateCode(ZwangineContext zwangineContext, String expression) throws SimpleParserException {
        ...
        // attachments
        if ("attachments".equals(function) || ifStartsWithReturnRemainder("attachment", function) != null) {
            String code = createCodeAttachments(zwangineContext, function);
            if (code != null) {
                return code;
            }
        }
    ...
    public String createCodeDirectly(String expression) throws SimpleParserException {
        if (ObjectHelper.isEqualToAny(expression, "body", "in.body")) {
            ...
            return "fromWorkflowId(exchange)";
        } else if (ObjectHelper.equal(expression, "routeId")) {
    ...
    private String createCodeAttachments(ZwangineContext zwangineContext, String function) {
        Optional<SimpleLanguageFunctionFactory> factory = ResolverHelper.resolveService(
                zwangineContext,
                zwangineContext.getZwangineContextExtension().getBootstrapFactoryFinder(),
                SimpleLanguageFunctionFactory.FACTORY + "/zwangine-attachments",
        ...
        return factory.get().createCode(zwangineContext, function, token.getIndex());
    }
    ...
    private String createCodeExpressionMisc(String function) {
        ...
        if (remainder != null) {
            ...
            if ("classic".equals(generator)) {
                sb.append("    UuidGenerator uuid = new org.zenithblox.support.ClassicUuidGenerator();\n");
                sb.append("return uuid.generateUuid();");
            } else if ("short".equals(generator)) {
                sb.append("    UuidGenerator uuid = new org.zenithblox.support.ShortUuidGenerator();\n");
                sb.append("return uuid.generateUuid();");
            } else if ("simple".equals(generator)) {
                sb.append("    UuidGenerator uuid = new org.zenithblox.support.SimpleUuidGenerator();\n");
                sb.append("return uuid.generateUuid();");
            } else if ("default".equals(generator)) {
                sb.append("    UuidGenerator uuid = new org.zenithblox.support.DefaultUuidGenerator();\n");
                sb.append("return uuid.generateUuid();");
            } else {
