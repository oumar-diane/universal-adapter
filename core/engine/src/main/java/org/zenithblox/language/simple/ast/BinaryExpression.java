...
import org.zenithblox.language.simple.types.BinaryOperatorType;
...
import org.zenithblox.support.builder.ValueBuilder;
...
public class BinaryExpression extends BaseSimpleNode {
    ...
    public Expression createExpression(ZwangineContext zwangineContext, String expression) {
        org.zenithblox.util.ObjectHelper.notNull(left, "left node", this);
        org.zenithblox.util.ObjectHelper.notNull(right, "right node", this);
        ...
        if (operator == BinaryOperatorType.EQ) {
            return createExpression(zwangineContext, leftExp, rightExp, PredicateBuilder.isEqualTo(leftExp, rightExp));
        } else if (operator == BinaryOperatorType.EQ_IGNORE) {
            return createExpression(zwangineContext, leftExp, rightExp, PredicateBuilder.isEqualToIgnoreCase(leftExp, rightExp));
        } else if (operator == BinaryOperatorType.GT) {
            return createExpression(zwangineContext, leftExp, rightExp, PredicateBuilder.isGreaterThan(leftExp, rightExp));
        } else if (operator == BinaryOperatorType.GTE) {
            return createExpression(zwangineContext, leftExp, rightExp,
                    PredicateBuilder.isGreaterThanOrEqualTo(leftExp, rightExp));
        } else if (operator == BinaryOperatorType.LT) {
            return createExpression(zwangineContext, leftExp, rightExp, PredicateBuilder.isLessThan(leftExp, rightExp));
        } else if (operator == BinaryOperatorType.LTE) {
            return createExpression(zwangineContext, leftExp, rightExp, PredicateBuilder.isLessThanOrEqualTo(leftExp, rightExp));
        } else if (operator == BinaryOperatorType.NOT_EQ) {
            return createExpression(zwangineContext, leftExp, rightExp, PredicateBuilder.isNotEqualTo(leftExp, rightExp));
            ...
            return createExpression(zwangineContext, leftExp, rightExp, PredicateBuilder.contains(leftExp, rightExp));
            ...
            return createExpression(zwangineContext, leftExp, rightExp, PredicateBuilder.containsIgnoreCase(leftExp, rightExp));
            ...
            return createIsExpression(zwangineContext, expression, leftExp, rightExp);
        } else if (operator == BinaryOperatorType.REGEX || operator == BinaryOperatorType.NOT_REGEX) {
            return createRegexExpression(zwangineContext, leftExp, rightExp);
        } else if (operator == BinaryOperatorType.IN || operator == BinaryOperatorType.NOT_IN) {
            return createInExpression(zwangineContext, leftExp, rightExp);
        } else if (operator == BinaryOperatorType.RANGE || operator == BinaryOperatorType.NOT_RANGE) {
            return createRangeExpression(zwangineContext, expression, leftExp, rightExp);
        } else if (operator == BinaryOperatorType.STARTS_WITH) {
            return createExpression(zwangineContext, leftExp, rightExp, PredicateBuilder.startsWith(leftExp, rightExp));
        } else if (operator == BinaryOperatorType.ENDS_WITH) {
            return createExpression(zwangineContext, leftExp, rightExp, PredicateBuilder.endsWith(leftExp, rightExp));
        }
    ...
    private Expression createIsExpression(
            final ZwangineContext zwangineContext, final String expression, final Expression leftExp, final Expression rightExp) {
        return new Expression() {
            ...
            public <T> T evaluate(Exchange exchange, Class<T> type) {
                ...
                Class<?> rightType = zwangineContext.getClassResolver().resolveClass(name);
                if (rightType == null) {
    ...
    private Expression createRegexExpression(
            final ZwangineContext zwangineContext, final Expression leftExp, final Expression rightExp) {
        return new Expression() {
            @Override
            public <T> T evaluate(Exchange exchange, Class<T> type) {
    ...
    private Expression createExpression(
            final ZwangineContext zwangineContext, final Expression left, final Expression right, final Predicate predicate) {
        return new Expression() {
            @Override
            public <T> T evaluate(Exchange exchange, Class<T> type) {
                boolean answer = predicate.matches(exchange);
