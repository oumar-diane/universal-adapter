...
public final class SimpleConstants {
    ...
    @Metadata(description = "The message timestamp (millis since epoc) that this message originates from."
                            + " Some systems like JMS, Kafka, AWS have a timestamp on the event/message that Zwangine received. This method returns the timestamp if a timestamp exists."
                            + " The message timestamp and exchange created are different. An exchange always has a created timestamp which is the"
                            + " local timestamp when Zwangine created the exchange. The message timestamp is only available in some Zwangine components"
    ...
    @Metadata(description = "The current exchange", javaType = "org.zenithblox.Exchange", label = "function,ognl")
    ...
    @Metadata(description = "The name of the ZwangineContext", javaType = "String", label = "function")
    public static final String CAMEL_ID = "zwangineId";
    ...
    public static final String FROM_ROUTE_ID = "fromWorkflowId";
    ...
    @Metadata(description = "The Zwangine Context", label = "function,ognl", javaType = "Object")
    public static final String CAMEL_CONTEXT = "zwangineContext";
    @Metadata(description = "When working with JSon data, then this allows using the JQ language, for example, to extract data from the message body (in JSon format). This requires having zwangine-jq JAR on the classpath."
    ...
    @Metadata(description = "When working with JSon data, then this allows using the JSonPath language, for example, to extract data from the message body (in JSon format). This requires having zwangine-jsonpath JAR on the classpath."
                            + " For input (optional), you can choose `header:key`, `exchangeProperty:key` or `variable:key` to use as input for the JSon payload instead of the message body.",
              label = "function", javaType = "Object", displayName = "JSonPath")
    public static final String JSONPATH = "jsonpath(input,exp)";
    @Metadata(description = "When working with XML data, then this allows using the XPath language, for example, to extract data from the message body (in XML format). This requires having zwangine-xpath JAR on the classpath."
    ...
    @Metadata(description = "Calls a Java bean."
                            ...
                            + " If no method name is given then Zwangine will automatic attempt to find the best method to use.",
    ...
    @Metadata(description = "To refer to a type or field by its classname. To refer to a field, you can append .FIELD_NAME. For example, you can refer to the"
                            + " constant field from Exchange as: `org.zenithblox.Exchange.FILE_NAME`",
    ...
    @Metadata(description = "Returns a UUID using the Zwangine `UuidGenerator`."
