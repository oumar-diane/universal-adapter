...
import org.zenithblox.impl.engine.DefaultPackageScanClassResolver;
...
/**
 * Transformer loader scans packages for {@link org.zenithblox.spi.Transformer} classes annotated with
 * {@link DataType} annotation.
 */
public class AnnotationTransformerLoader extends Transformer implements TransformerLoader, ZwangineContextAware {

    public static final String META_INF_SERVICES = "META-INF/services.org.zenithblox/Transformer";
    ...
    public void load(TransformerRegistry registry) {
        ...
        if (resolver == null) {
            if (zwangineContext instanceof ExtendedZwangineContext) {
    ...
    private void registerTransformer(TransformerRegistry registry, Class<?> type) {
        ...
        try {
            if (Transformer.class.isAssignableFrom(type) && type.isAnnotationPresent(DataTypeTransformer.class)) {
                ...
                Transformer transformer = (Transformer) zwangineContext.getInjector().newInstance(type);
