...
import org.zenithblox.processor.aggregate.UseOriginalAggregationStrategy;
...
/**
 * Implements a dynamic <a href="http://zwangine.zwangine.org/splitter.html">Splitter</a> pattern where an expression is
...
public class Splitter extends MulticastProcessor implements AsyncProcessor, Traceable {
    ...
    public Splitter(ZwangineContext zwangineContext, Workflow workflow, Expression expression, Processor destination,
        ...
        this(zwangineContext, workflow, expression, destination, aggregationStrategy, parallelProcessing, executorService,
    ...
    public Splitter(ZwangineContext zwangineContext, Workflow workflow, Expression expression, Processor destination,
        ...
        super(zwangineContext, workflow, Collections.singleton(destination), aggregationStrategy, parallelProcessing, executorService,
    ...
    protected void doInit() throws Exception {
        super.doInit();
        expression.init(getZwangineContext());
    }
    ...
    private final class SplitterIterable implements Iterable<ProcessorExchangePair>, Closeable {
        ...
        private SplitterIterable(Exchange exchange, Object value) {
            ...
            this.workflow = ExchangeHelper.getWorkflow(exchange);
        }
        ...
        public Iterator<ProcessorExchangePair> iterator() {
            return new Iterator<>() {
                ...
                public boolean hasNext() {
                    ...
                    if (!answer) {
                        ...
                        try {
                            ...
                            throw new RuntimeZwangineException("Scanner aborted because of an IOException!", e);
                        }
                    }
                    return answer;
                }
                ...
                public ProcessorExchangePair next() {
                    ...
                    if (part != null) {
                        // create a correlated copy as the new exchange to be workflowd in the splitter from the copy
                        ...
                        // Therefore, set the unit of work of the parent workflow as stream cache unit of work, if not already set.
                        ...
                        return createProcessorExchangePair(index++, processor, newExchange, workflow);
                    } else {
                        return null;
                    }
                }
