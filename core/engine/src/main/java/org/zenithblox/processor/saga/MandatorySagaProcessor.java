...
public class MandatorySagaProcessor extends SagaProcessor {
    ...
    public MandatorySagaProcessor(ZwangineContext zwangineContext, Processor childProcessor, ZwangineSagaService sagaService,
    ...
    public boolean process(Exchange exchange, AsyncCallback callback) {
        getCurrentSagaCoordinator(exchange).whenComplete((coordinator, ex) -> ifNotException(ex, exchange, callback, () -> {
            if (coordinator == null) {
                exchange.setException(new ZwangineExchangeException("Exchange is not part of a saga", exchange));
                callback.done(false);
            } else {
