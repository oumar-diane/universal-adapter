...
/**
 * An implementation of the <a href="http://zwangine.zwangine.org/resequencer.html">Resequencer</a> which can reorder
 * messages within a batch.
 */
public class Resequencer extends AsyncProcessorSupport implements Navigate<Processor>, IdAware, WorkflowIdAware, Traceable {
    ...
    public Resequencer(ZwangineContext zwangineContext, Processor processor, Expression expression) {
        this(zwangineContext, processor, createSet(expression, false, false), expression);
    }

    public Resequencer(ZwangineContext zwangineContext, Processor processor, Expression expression,
                       boolean allowDuplicates, boolean reverse) {
        this(zwangineContext, processor, createSet(expression, allowDuplicates, reverse), expression);
    }

    public Resequencer(ZwangineContext zwangineContext, Processor processor, Set<Exchange> collection, Expression expression) {
        ...
        try {
            ...
            if (!isValid(exchange)) {
                if (isIgnoreInvalidExchanges()) {
                    ...
                    throw new ZwangineExchangeException("Exchange is not valid to be used by the BatchProcessor", exchange);
                }
            } else {
    ...
    private class BatchSender extends Thread {
        ...
        BatchSender() {
            super(zwangineContext.getExecutorServiceManager().resolveThreadName("Batch Sender"));
