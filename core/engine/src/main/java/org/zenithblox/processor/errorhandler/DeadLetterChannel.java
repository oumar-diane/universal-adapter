...
/**
 * Implements a <a href="http://zwangine.zwangine.org/dead-letter-channel.html">Dead Letter Channel</a> after attempting to
...
public class DeadLetterChannel extends RedeliveryErrorHandler {
    ...
    private static final ZwangineLogger DEFAULT_LOGGER
            = new ZwangineLogger(LoggerFactory.getLogger(DeadLetterChannel.class), LoggingLevel.ERROR);
     ...
     */
    public DeadLetterChannel(ZwangineContext zwangineContext, Processor output, ZwangineLogger logger, Processor redeliveryProcessor,
        ...
        super(zwangineContext, output, logger != null ? logger : DEFAULT_LOGGER, redeliveryProcessor, redeliveryPolicy, deadLetter,
              deadLetterUri,
    ...
    public ErrorHandler clone(Processor output) {
        DeadLetterChannel answer = new DeadLetterChannel(
                zwangineContext, output, logger, redeliveryProcessor, redeliveryPolicy, deadLetter, deadLetterUri,
