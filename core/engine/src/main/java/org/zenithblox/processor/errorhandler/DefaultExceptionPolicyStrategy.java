...
/**
 * The default strategy used in Zwangine to resolve the {@link ExceptionPolicyKey} that should handle the thrown exception.
 * <p/>
 ...
 * exchange using the {@link #matchesWhen(ExceptionPolicyKey, org.zenithblox.Exchange)} method. This can be used to
...
public class DefaultExceptionPolicyStrategy implements ExceptionPolicyStrategy {
    ...
    public ExceptionPolicyKey getExceptionPolicy(
        ...
        Set<ExceptionPolicyKey> workflowScoped = new LinkedHashSet<>();
        Set<ExceptionPolicyKey> contextScoped = new LinkedHashSet<>();

        // split policies into workflow and context scoped
        initWorkflowAndContextScopedExceptionPolicies(exceptionPolicies, workflowScoped, contextScoped);

        // at first check workflow scoped as we prefer them over context scoped
        ...
        while (!exactMatch && it.hasNext()) {
            ...
            exactMatch = findMatchedExceptionPolicy(workflowScoped, exchange, it.next(), candidates);
        }
        ...
        if (candidates.isEmpty()) {
            ...
            // return the first in the map as its sorted and we checked workflow scoped first, which we prefer
            return candidates.values().iterator().next();
        }
    }

    private static void initWorkflowAndContextScopedExceptionPolicies(
            Set<ExceptionPolicyKey> exceptionPolicies,
            Set<ExceptionPolicyKey> workflowScoped,
            Set<ExceptionPolicyKey> contextScoped) {

        // loop through all the entries and split into workflow and context scoped
        for (ExceptionPolicyKey entry : exceptionPolicies) {
            if (entry.getWorkflowId() != null) {
                workflowScoped.add(entry);
            } else {
    ...
    private boolean findMatchedExceptionPolicy(
        ...
        for (ExceptionPolicyKey type : exceptionPolicies) {
            ...
            // if ExceptionPolicy is workflow scoped then the current workflow (Exchange) must match
            // so we will not pick an ExceptionPolicy from another workflow
            String typeWorkflow = type.getWorkflowId();
            if (exchange != null && ObjectHelper.isNotEmpty(typeWorkflow)) {
                String workflow = ExchangeHelper.getWorkflowId(exchange);
                if (workflow != null && !workflow.equals(typeWorkflow)) {
                    if (LOG.isTraceEnabled()) {
                        LOG.trace("The type is scoped for workflow: {} however Exchange is at workflow: {}", typeWorkflow, workflow);
                    }
                    continue;
                }
            }
        ...
        if (candidate != null) {
            if (!candidates.containsKey(candidateDiff)) {
                ...
                // for example we check workflow scope before context scope (preferring workflow scopes)
                if (LOG.isTraceEnabled()) {
    ...
    /**
     * Strategy method creating the iterator to walk the exception in the order Zwangine should use for find the
