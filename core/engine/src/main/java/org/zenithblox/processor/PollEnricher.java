 ...
 * <p/>
 * Uses a {@link org.zenithblox.PollingConsumer} to obtain the additional data as opposed to {@link Enricher} that
 * uses a {@link org.zenithblox.Producer}.
 *
 * @see PollProcessor
 * @see Enricher
 */
public class PollEnricher extends AsyncProcessorSupport implements IdAware, WorkflowIdAware, ZwangineContextAware {
    ...
    public boolean process(Exchange exchange, AsyncCallback callback) {
        try {
            ...
            exchange.setException(new ZwangineExchangeException("Error during pre poll check", exchange, e));
            callback.done(true);
            return true;
        }
        ...
        try {
            ...
            exchange.setException(new ZwangineExchangeException("Error during poll", exchange, e));
            callback.done(true);
            return true;
        } finally {
     ...
     * <p/>
     * Is currently used to prevent doing poll enrich from a file based endpoint when the current workflow also started
    ...
    protected void doBuild() throws Exception {
        if (consumerCache == null) {
            ...
            consumerCache = new DefaultConsumerCache(this, zwangineContext, cacheSize);
