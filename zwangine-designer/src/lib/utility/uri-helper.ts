import { getParsedValue } from './get-parsed-value.ts';
import { getValue } from './get-value.ts';
import { isDefined } from './is-defined.ts';
import {IElementLookupResult} from "@/core";

export type ParsedParameters = Record<string, string | boolean | number>;

/**
 * Helper class for working with Ua URIs
 */
export class UriHelper {
    ...
    // get the URI
    static getUriString<T>(dslAbstractRepresentation: T | undefined | null): string | undefined {
        /** For string-based processor dslAbstractRepresentation, we can return the dslAbstractRepresentation itself */
        if (typeof dslAbstractRepresentation === 'string' && dslAbstractRepresentation !== '') {
            return dslAbstractRepresentation;
        }

        const uriString = getValue(dslAbstractRepresentation, 'uri');

        /** For object-based processor dslAbstractRepresentation, we can return the `uri` property if not empty */
    ...
    // get the semantic name from the parameters
    static getSemanticString<T>(
        elementLookup: IElementLookupResult,
        value: T | undefined | null,
    ): string | undefined {
        /** For string-based processor dslAbstractRepresentation, we return undefined */
        if (!isDefined(value) || typeof value === 'string') {
            return undefined;
        }

        switch (elementLookup.componentName) {
            case 'direct':
