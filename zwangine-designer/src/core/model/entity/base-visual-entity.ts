import {BaseEntity, EntityType} from "./base-entity.ts";
import {NodeLabelType, SchemaDefinition} from "../schema/schema.ts";
import {DefinedComponent} from "../catalog/catalog-index.ts";


/**
 * BaseVisualEntity
 *
 * This interface is used to represent a visual entity.
 * f.i., etc.
 * All dedicated universal integration entity code should be implemented using this interface.
 */
export interface BaseVisualEntity extends BaseEntity {
    ...
    /** Returns fields that should be omitted when configuring this entity */
    getOmitFormFields: () => string[];

    /** Given a path, update the model */
    updateModel(path: string | undefined, value: unknown): void;

    /** Add a step to the underlying entity */
    addStep: (options: {
    ...
    /** Remove the step at a given path from the underlying entity */
    ...
    /** Generates a IVisualizationNode from the underlying entity */
    toVizNode: () => IVisualizationNode;
}

export interface BaseVisualEntityConstructor {
 ...
 * Usually, each IVisualizationNode represent a component in a {@link BaseVisualEntity}.
 *
 * This interface shouldn't know anything about the specific {@link BaseVisualEntity} but should be used by the {@link BaseVisualEntity} internally like Adapter pattern .
 */
export interface IVisualizationNode<T extends IVisualizationNodeData = IVisualizationNodeData> {
    ...
    getNodeLabel(nodeLabelType?:NodeLabelType): string;
...
/**
 * UI related data for the visualization node
 *
 * **/
