...
import './custom-auto-fields.scss';
import {CanvasFormTabsContext, FilteredFieldContext } from '@/providers';
import {getFieldGroups, getFilteredProperties, isDefined } from '@/lib/utility';
import { CustomExpandableSection } from './customField/custom-expandable-section';
import {NoFieldFound} from "@/components/form/no-field-found.tsx";
import {SchemaDefinition} from "@/core/model/schema/schema.ts";
import {CatalogKind} from "@/core/model/catalog/catalog-kind.ts";
...
export function CustomAutoFields({
    ...
    const schemaObject = isDefined(fields)
        ...
        : (rootField as SchemaDefinition['schema']).properties;

    const filteredProperties = getFilteredProperties(
        schemaObject as SchemaDefinition['schema']['properties'],
        cleanQueryTerm,
        omitFields,
    );
    ...
    if (
        ...
        const comment = (rootField as SchemaDefinition['schema'])['$comment'] ?? '';
