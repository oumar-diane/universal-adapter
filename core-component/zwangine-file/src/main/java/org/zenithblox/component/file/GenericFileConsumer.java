...
import org.zenithblox.support.EmptyAsyncCallback;
import org.zenithblox.support.MessageHelper;
import org.zenithblox.support.ScheduledBatchPollingConsumer;
...
public abstract class GenericFileConsumer<T> extends ScheduledBatchPollingConsumer {
    ...
    protected boolean processExchange(final Exchange exchange) {
        ...
        // we can begin processing the file so update file headers on the Zwangine
        // message
     ...
     *
     * @param file    the file
     * @param message the Zwangine message to update its headers
     */
     ...
     * </ul>
     */
    protected boolean isMatchedHiddenFile(Supplier<GenericFile<T>> file, String name, boolean isDirectory) {
        // folders/names starting with dot is always skipped (eg. ".", ".zwangine",
        // ".zwangineLock")
        if (name.startsWith(".")) {
            return false;
        }
    ...
    protected boolean isMatched(
        ...
        if (endpoint.getDoneFileName() != null) {
            ...
            String doneFileName = endpoint.createDoneFileName(absoluteFilePath);
    ...
    protected String evaluateFileExpression(Exchange exchange) {
        ...
        if (exchange.getException() != null) {
            throw RuntimeZwangineException.wrapRuntimeZwangineException(exchange.getException());
        }
        return result;
    }
    ...
    protected void doInit() throws Exception {
        super.doInit();
        // inject ZwangineContext before starting as it may be needed
        if (processStrategy instanceof ZwangineContextAware zwangineContextAware) {
            zwangineContextAware.setZwangineContext(getEndpoint().getZwangineContext());
        }
    }
