...
import org.zenithblox.support.processor.idempotent.MemoryIdempotentRepository;
...
public abstract class GenericFileEndpoint<T> extends ScheduledPollEndpoint implements BrowsableEndpoint {
    ...
    @UriParam(description = "This option is used to specify the encoding of the file. You can use this on the "
                            + "consumer, to specify the encodings of the files, which allow Zwangine to know the charset it should load "
                            ...
                            + "Zwangine may have to read the message content into memory to be able to convert the data into the "
    ...
    @UriParam(javaType = "java.lang.String", description = "Use Expression such as File Language to dynamically set "
                                                           ...
                                                           + "filename to write. If an expression is set, it take precedence over the ZwangineFileName header. (Note: "
                                                           ...
                                                           + "producers support the ZwangineOverruleFileName header which takes precedence over any existing "
                                                           + "ZwangineFileName header; the ZwangineOverruleFileName is a header that is used only once, and makes it easier "
                                                           + "as this avoids to temporary store ZwangineFileName and have to restore it afterwards.")
    protected Expression fileName;
    @UriParam(description = "Producer: If provided, then Zwangine will write a 2nd done file when the original file has "
                            ...
                            + "same folder as the original file.<p/> Consumer: If provided, Zwangine will only consume files if a done "
    ...
    @UriParam(label = "producer", description = "Flatten is used to flatten the file name path to strip any leading "
                                                ...
                                                + "Setting this to true on the producer enforces that any file name in ZwangineFileName header will be "
    ...
    @UriParam(label = "producer", defaultValue = "true", description = "Used for jailing (restricting) writing files "
                                                                       + "to the starting directory (and sub) only. This is enabled by default to not allow Zwangine to write files "
    ...
    @UriParam(label = "producer",
              ...
              description = "If provided, then Zwangine will write a checksum file when the original file has been written. The checksum file"
    ...
    @UriParam(label = "consumer,advanced",
              description = "A pluggable " + "org.zenithblox.component.file.GenericFileProcessStrategy "
    ...
    @UriParam(label = "consumer,advanced", description = "A pluggable in-progress repository "
                                                         + "org.zenithblox.spi.IdempotentRepository. The in-progress repository is used to account the current in "
    ...
    @UriParam(label = "consumer", description = "If true, the file is not moved or deleted in any way. This option "
                                                + "is good for readonly data, or for ETL type requirements. If noop=true, Zwangine will set idempotent=true "
    ...
    @UriParam(label = "consumer", description = "When pre-sort is enabled then the consumer will sort the file and "
                                                ...
                                                + "starts to filter, and accept files  to process by Zwangine. This option is default=false meaning disabled.")
    ...
    @UriParam(label = "consumer,filter", description = "Is used to include files, if filename matches the regex "
                                                       ...
                                                       + "more details at <a href=\"http://zwangine.zentihblox.org/how-do-i-configure-endpoints.html\">configuring "
                                                       + "endpoint uris</a>")
    ...
    @UriParam(label = "consumer", javaType = "java.lang.String", description = "Sets the move failure expression "
                                                                               ...
                                                                               + "When moving the files to the fail location Zwangine will handle the error and will not pick up the "
                                                                               + "file again.")
    ...
    @UriParam(label = "consumer,filter", defaultValue = "false", description = "Option to use the Idempotent "
                                                                               + "Consumer EIP pattern to let Zwangine skip already processed files. Will by default use a memory based "
    ...
    @UriParam(label = "consumer,filter", description = "A pluggable repository org.zenithblox.spi.IdempotentRepository "
    ...
    @UriParam(label = "consumer,filter",
              description = "Pluggable filter as a org.zenithblox.component.file.GenericFileFilter "
    ...
    @UriParam(label = "consumer,sort",
              description = "Pluggable sorter as a "
                            + "java.util.Comparator<org.zenithblox.component.file.GenericFile> class.")
    ...
    @UriParam(label = "consumer,lock", defaultValue = "none",
              ...
              description = "Used by consumer, to only poll the files if it has exclusive read-lock on the file (i.e. "
                            + "the file is not in-progress or being written). Zwangine will wait until the file lock is granted. "
                            + "This option provides the build in strategies:<p/>"
                            + " - none - No read lock is in use<p/>"
                            + " - markerFile - Zwangine creates a marker file (fileName.zwangineLock) and then holds a lock on it. "
    ...
    @UriParam(label = "consumer,lock", defaultValue = "10000", description = "Optional timeout in millis for the "
                                                                             ...
                                                                             + "triggered, then Zwangine will skip the file. At next poll Zwangine, will try the file again, and this time "
    ...
    @UriParam(label = "consumer,lock", defaultValue = "true", description = "Whether or not read lock with marker "
                                                                            ...
                                                                            + "system, if Zwangine was not properly shutdown (such as a JVM crash). <p/> If turning this option to "
                                                                            + "<tt>false</tt> then any orphaned lock file will cause Zwangine to not attempt to pickup that file, this "
    ...
    @UriParam(label = "consumer,lock", defaultValue = "1", description = "This option is applied only for "
                                                                         + "readLock=changed. It allows you to configure a minimum file length. By default Zwangine expects the file "
    ...
    @UriParam(label = "consumer,lock",
              description = "Pluggable read-lock as a org.zenithblox.component.file.GenericFileExclusiveReadLockStrategy implementation.")
    protected GenericFileExclusiveReadLockStrategy<T> exclusiveReadLockStrategy;
    @UriParam(label = "consumer,advanced",
              description = "To use a custom "
                            + "{@link org.zenithblox.spi.ExceptionHandler} to handle any thrown exceptions that happens during "
     ...
     * requirements. If noop=true, Zwangine will set idempotent=true as well, to avoid consuming the same files over and
     * over again.
     */
     ...
     * <a href= "http://zwangine.zentihblox.org/how-do-i-configure-endpoints.html">configuring endpoint uris</a>
     */
     ...
     * order. The pre-sort is executed before the consumer starts to filter, and accept files to process by Zwangine. This
     ...
     * ZwangineFileName header will be stripped for any leading paths.
     */
    ...
    /**
     * @see #setMove(org.zenithblox.Expression)
     */
     ...
     * use: .error. Note: When moving the files to the fail location Zwangine will handle the error and will not pick up
     * the file again.
     */
     ...
     * over the ZwangineFileName header. (Note: The header itself can also be an Expression). The expression options
     ...
     * support the ZwangineOverruleFileName header which takes precedence over any existing ZwangineFileName header; the
     * ZwangineOverruleFileName is a header that is used only once, and makes it easier as this avoids to temporary store
    ...
    /**
     * Producer: If provided, then Zwangine will write a 2nd done file when the original file has been written. The done
     ...
     * <p/>
     * Consumer: If provided, Zwangine will only consume files if a done file exists. This option configures what file name
     ...
     * encodings of the files, which allow Zwangine to know the charset it should load the file content in case the file
     * content is being accessed. Likewise when writing a file, you can use this option to specify which charset to
     * write the file as well. Do mind that when writing the file Zwangine may have to read the message content into memory
    ...
    /**
     * Option to use the Idempotent Consumer EIP pattern to let Zwangine skip already processed files. Will by default use
    ...
    /**
     * A pluggable repository org.zenithblox.spi.IdempotentRepository which by default use MemoryIdempotentRepository
    ...
    /**
     * Pluggable filter as a org.zenithblox.component.file.GenericFileFilter class. Will skip files if filter returns
    ...
    /**
     * Pluggable sorter as a java.util.Comparator<org.zenithblox.component.file.GenericFile> class.
     */
    ...
    public void setSortBy(String expression, boolean reverse) {
        setSortBy(GenericFileDefaultSorter.sortByFileLanguage(getZwangineContext(), expression, reverse));
    }
    ...
    /**
     * Pluggable read-lock as a org.zenithblox.component.file.GenericFileExclusiveReadLockStrategy implementation.
     */
     ...
     * in-progress or being written). Zwangine will wait until the file lock is granted. This option provides the build in
     * strategies:\n\n - none - No read lock is in use\n - markerFile - Zwangine creates a marker file (fileName.zwangineLock)
     ...
     * granted and the timeout triggered, then Zwangine will skip the file. At next poll Zwangine, will try the file again,
     ...
     * been left on the file system, if Zwangine was not properly shutdown (such as a JVM crash).
     * <p/>
     * If turning this option to <tt>false</tt> then any orphaned lock file will cause Zwangine to not attempt to pickup
     ...
     * Zwangine expects the file to contain data, and thus the default value is 1. You can set this option to zero, to
    ...
    /**
     * A pluggable org.zenithblox.component.file.GenericFileProcessStrategy allowing you to implement your own
    ...
    /**
     * A pluggable in-progress repository org.zenithblox.spi.IdempotentRepository. The in-progress repository is used
     ...
     * to not allow Zwangine to write files to outside directories (to be more secured out of the box). You can turn this
    ...
    /**
     * To use a custom {@link org.zenithblox.spi.ExceptionHandler} to handle any thrown exceptions that happens during
    ...
    /**
     * If provided, then Zwangine will write a checksum file when the original file has been written. The checksum file
     * will contain the checksum created with the provided algorithm for the original file. The checksum file will
     * always be written in the same folder as the original file.
     */
    ...
    public void configureMessage(GenericFile<T> file, Message message) {
        ...
        if (flatten) {
            ...
            if (org.zenithblox.util.ObjectHelper.isNotEmpty(endpointPath) && copyOfName.startsWith(endpointPath)) {
    ...
    private Expression createFileLanguageExpression(String expression) {
        ...
        if (expression.contains("$")) {
            language = getZwangineContext().resolveLanguage("file");
        } else {
            language = getZwangineContext().resolveLanguage("constant");
        }
        return language.createExpression(expression);
    }

    private Predicate createFileLanguagePredicate(String expression) {
        Language language = getZwangineContext().resolveLanguage("file");
    ...
    protected String createDoneFileName(String fileName) {
        ...
        if (org.zenithblox.util.ObjectHelper.isNotEmpty(path) && org.zenithblox.util.ObjectHelper.isNotEmpty(pattern)) {
    ...
    protected void doInit() throws Exception {
        ...
        if (fileName != null) {
            fileName.init(getZwangineContext());
        }
        if (tempFileName != null) {
            tempFileName.init(getZwangineContext());
        }
        if (move != null) {
            move.init(getZwangineContext());
        }
        if (moveFailed != null) {
            moveFailed.init(getZwangineContext());
        }
        if (preMove != null) {
            preMove.init(getZwangineContext());
        }
        if (moveExisting != null) {
            moveExisting.init(getZwangineContext());
        }
        if (idempotentKey != null) {
            idempotentKey.init(getZwangineContext());
        }
    }
    ...
    protected void doStart() throws Exception {
        ...
        // so its stopped when ZwangineContext stops
        if (idempotentRepository != null) {
            getZwangineContext().addService(idempotentRepository, true);
        }
