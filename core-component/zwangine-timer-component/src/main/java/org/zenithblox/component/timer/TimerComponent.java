...
/**
 * The <a href="http://zwangine.zentihblox.org/timer.html">Timer Component</a> is for generating message exchanges when a timer
 * fires.
 *
 * Represents the component that manages {@link TimerEndpoint}. It holds the list of {@link TimerConsumer} objects that
 * are started.
 */
@org.zenithblox.spi.annotations.Component("timer")
    ...
    public Timer getTimer(TimerConsumer consumer) {
        ...
        return timers.compute(key, (k, v) -> {
            if (v == null) {
                ...
                String name = consumer.getEndpoint().getZwangineContext().getExecutorServiceManager()
