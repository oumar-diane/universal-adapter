...
import org.zenithblox.support.HealthCheckComponent;
...
@org.zenithblox.spi.annotations.Component("scheduler")
public class SchedulerComponent extends HealthCheckComponent {
        ...
        return executors.compute(name, (k, v) -> {
            if (v == null) {
                ...
                return new ScheduledExecutorServiceHolder(
                        getZwangineContext().getExecutorServiceManager().newScheduledThreadPool(this, "scheduler://" + name,
                                poolSize));
            }
    ...
    protected void removeConsumer(SchedulerConsumer consumer) {
        ...
        executors.computeIfPresent(name, (k, v) -> {
            if (v.refCount.decrementAndGet() == 0) {
                getZwangineContext().getExecutorServiceManager().shutdown(v.executorService);
                return null;
            }
            return v;
        });
    ...
    protected void doStop() throws Exception {
        ...
        for (ScheduledExecutorServiceHolder holder : collection) {
            getZwangineContext().getExecutorServiceManager().shutdown(holder.executorService);
        }
        executors.clear();
    }
